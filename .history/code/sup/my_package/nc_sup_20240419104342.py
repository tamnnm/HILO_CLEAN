...
def cut_co(dts, ulat, dlat, ulon, dlon, name = [None, None], full: bool = False, data: bool = True):
    """
    This function cuts a dataset according to a specified grid and time.

    Args:
        dts (xarray.Dataset): The input dataset to be cut.
        ulat (float): Upper limit for latitude.
        dlat (float): Lower limit for latitude.
        ulon (float): Upper limit for longitude.
        dlon (float): Lower limit for longitude.
    ...
    Process:
        1. Find the names of the latitude and longitude dimensions in the dataset.


    Returns:
        dict: A dictionary containing the names of the latitude and longitude dimensions, the data of the cut points, and optionally the full dataset after cutting.
    """
...
def cut_points(dts, points: Tuple[Tuple[float,float]], name = [None, None], full: bool = False, data: bool = True):
    ...
    # In case the points are not float
