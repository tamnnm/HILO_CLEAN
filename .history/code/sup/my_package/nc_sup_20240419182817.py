...
def cut_co(dts, dlat, ulat, dlon, ulon, name=[None, None], full: bool = False, data: bool = True):
    result = {}
    if name:
    ...
    if [lat_co_name, lon_co_name]:
        raise KeyError(f"Check the name of this dimension {name}.\n")
    print(lat_co_name, lon_co_name)
...
def cut_points(dts, points: Tuple[Tuple[int, int]], name=[None, None], full: bool = False, data: bool = True):
    result = {}
    if name:
        lat_co_name = find_name(dts, name[0], fname_group='lat', opt="coords")
        lon_co_name = find_name(dts, name[1], fname_group='lon', opt="coords")
    if [lat_co_name, lon_co_name].count(None) >= 1:
        raise KeyError(f"Check the name of this dimension {name}.\n")
...
def cut_time(dts, ss: Optional[Literal["DJF", "MAM", "JJA", "SON"]] = None, name=None,
    ...
    def date_indentify(value):
        ...
        format = formats[f'{len(date_split)}'][0] if int(
            date_split[0]) > 12 else formats[f'{len(date_split)}'][1]
    ...
    # we ignore end_date
        ...
        cond = (time_co.dt.month == date_start[0]) if len_start == 1 else (
            (time_co.dt.month == date_start[0]) & (time_co.dt.day == date_start[1]))
    ...
    if ss is not None and format_start == formats[f'{len_start}'][0]:
        ...
        if ss == "DJF" and fin_yr_start < fin_yr_end:
            ...
            cond_ss_1 = ((time_co.dt.month <= 2) & (
                time_co.dt.year > fin_yr_start))
            cond_ss_2 = ((time_co.dt.month == 12) &
                         (time_co.dt.year < fin_yr_end))
...
def calculate_mean(final_dts, dim):
    ...
    if dim not in list(final_dts.dims):
        dim = next((dim_name for dim_name in coord_name[dim] if dim_name in list(
            final_dts.dims)), None)
    if dim is None:
        raise KeyError(f"Dimension {dim} is not in the dataset")
    result['name'] = dim
        ...
        if "all" in dim_mean:
            ...
            for dimension in dim_mean:
                ...
                if list_dim_ind is None:
                    raise KeyError(
                        f"Dimension {dimension} is not in the dataset")
