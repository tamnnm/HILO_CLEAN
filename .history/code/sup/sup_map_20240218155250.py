...
Data_path = "/work/users/tamnnm/code/city_list_obs/city/"
os.chdir(Data_path)
city_name = []
city_start_year = []
city_end_year = []
...
shp_path = "/work/users/tamnnm/map/vnm/full_shp/vnm_admbnda_adm0_gov_20200103.shp"
...
for i in range(4,2,-1):
    if number_file%i == 0:
        ...
        continue
fig,axs = plt.subplots(no_hor,no_ver,subplot_kw=dict(projection=ccrs.PlateCarree()),sharex=True, sharey=True, gridspec_kw={'hspace': 0, 'wspace': 0})
...
#"""
    ...
    ax.grid(linewidth=1, color='gray', alpha=0.5,
                    linestyle='--')
    ax.text(110,23,var_name,color='black',fontsize=23)
#"""
    ...
    ins = ax.inset_axes([0.75,0.4,0.2,0.2])
    p = ins.bar([1,2,3], weight, color = color, tick_label = name, width=0.5, bottom=1)
fig.tight_layout()
fig.savefig('/work/users/tamnnm/code/image/map_ob_image.jpg',format="jpg",dpi=1000)
#"""
