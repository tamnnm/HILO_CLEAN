...
for file_name in sorted(os.listdir(dat_thay)):
    ...
    with open(file_full, 'r') as file:
        ...
        #     last_year = lines[-32].strip()
        #     with open('city_list_obs/city/rain_city.txt', 'r') as file:
        #         lines = file.readlines()
        #         for line in lines:
        ...
        # Creat list of [city name, first year, last year, lat, lon]
        [name_element[1], first_year, last_year]
...
def f_write(city_list,file_name):
    with open(f'city_list_obs/city/{file_name}.txt','w') as file:
