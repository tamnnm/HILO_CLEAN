...
link = "/data/projects/REMOSAT/tamnnm/wrf_data/netcdf/para/para_167"
fig, ax = plt.subplots()

cera = xr.open_dataset(
    link+"/mean_cera_167.nc")['var167'].sel(lat=21.0, lon=106.0)
era = xr.open_dataset(link+"/mean_era_228.nc")['var167'].sel(lat=21.0, lon=106.0)
era_5 = xr.open_dataset(
    link+"/mean_era5_167")['var167'].sel(lat=21.25, lon=106.25)
noaa = xr.open_dataset(
    link+"/mean_noaa_167.nc")['air'].sel(lat=21.0, lon=107.0) * 2
obs = xr.open_dataset(link+"/mean_obs_167.nc")['T2m'].sel(no_station=61)
