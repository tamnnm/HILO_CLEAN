...
cera = xr.open_dataset(link+"/cera_228.nc")['tp'].sel(lat=16.0, lon=107.0)
era = xr.open_dataset(link+"/era_228.nc")['var228'].sel(lat=16.0, lon=107.0)
era_5 = xr.open_dataset(
    link+"/era5_228.nc")['var228'].sel(lat=16.25, lon=107.25)
noaa = xr.open_dataset(link+"/noaa_228.nc")['apcp'].sel(lat=16.0, lon=107.0)
obs = xr.open_dataset(link+"/obs_228.nc")['R'].sel(no_station=0)

reference_date = '1940-01-01 07:00:00.0'
cera['time'] = pd.date_range(
    start=reference_date, periods=cera.sizes['time'], freq="2H")
print(era['time'])
