...
cera = xr.open_dataset(link+"/cera_228.nc")['tp'].slice(lat=16.0, lon=107.0)
era = xr.open_dataset(link+"/era_228.nc")['var228'].slice(lat=16.0, lon=107.0)
era_5 = xr.open_dataset(link+"/era5_228.nc")['var228'].slice(lat=16.25, lon=107.25)
noaa = xr.open_dataset(link+"/noaa_228.nc")['var228'].slice(lat=16.0, lon=107.0)
obs = xr.open_dataset(link+"/obs_228.nc")['var228'].slice(no_station=0)

cera['tp'].plot(ax=ax, label='cera')
era['var228'].plot(ax=ax, label='era')
era_5['var228'].plot(ax=ax, label='era_5')
noaa['apcp'].plot(ax=ax, label='noaa')
obs['R'].plot(ax=ax, label='obs')
