    ...
    if yr == None:
        ...
        if mon == None:
            ...
            if ss == None:
                ...
                if ss == "DJF":
                    cond = (time_co.dt.month <= 2) & (
                        time_co.dt.year == (int(yr)+1))
                    cond2 = (time_co.dt.month == 12) & (
                        time_co.dt.year == int(yr))
            ...
            time_final = time_co[(time_co.dt.year == int(yr))
                                 & (time_co.dt.month == mon)]
        return time_final
...
def cut_mlp(dts, time_data=None, lat_data=None, lon_data=None, lev_data=None):
    if time_data != None:
        ...
        final_time = dts
    if lat_data != None and lon_data != None:
        try:
            final_latlon = final_time.sel(lon=lon_data, lat=lat_data)
        except:
            final_latlon = final_time.sel(
                longitude=lon_data, latitude=lat_data)
    else:
        final_latlon = final_time

    if lev_data != None:
        final_dts = final_latlon.sel(lev_data=lev_data)
    else:
