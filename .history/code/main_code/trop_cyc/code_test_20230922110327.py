...
is_ocean = sst.isel(time=0).squeeze('time').pipe(lambda x: x*0 == 0)
...
if os.path.exists(ofile):
    ...
    chi = entropy_deficit(
        ...
        RHb=rh2m,
        p_m=p_m,
        Tm=Tm_600.squeeze(dim='isobaricInhPa'),
        RHm=rh_600.squeeze(dim='isobaricInhPa')/100
    ).where(is_ocean)
    """
...
if os.path.exists(ofile):
    ...
    eta = absolute_vorticity(
        vort850=vort_850.squeeze(dim='isobaricInhPa'),
        lat=geo_co[0]
    )
    """
...
if os.path.exists(ofile):
    ...
    H = cut_mlp(rh, level_600).squeeze(dim='isobaricInhPa')
