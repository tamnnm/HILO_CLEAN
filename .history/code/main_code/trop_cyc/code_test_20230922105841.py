...
for i, z_p in enumerate(p_test):
    ...
    for day in np.arange(1, 2):
        ...
        data_test = cut_mlp(Tm, time_data=time_co,
                            geo_data=latlon_co, level_data=level_co, dim_mean=['time', dim='isobaricInhPa'], dim_plot=['geo'])  # even though there is one level butdo this so that the data array is 2 dimension
...
if os.path.exists(ofile):
    ...
    chi = entropy_deficit(
        ...
        RHb=rh2m,
        p_m=p_m,
        Tm=Tm_600.drop(dim=dim='isobaricInhPa'),
        RHm=rh_600.drop(dim=dim='isobaricInhPa')/100
    ).where(is_ocean)
    """
...
if os.path.exists(ofile):
    ...
    chi_sb = entropy_deficit(
        ...
        RHb=rh2m,
        p_m=p_m,
        Tm=Tm_600.drop(dim='isobaricInhPa'),
        RHm=rh_600.drop(dim='isobaricInhPa')/100,
        forGPI2010=True
    ).where(is_ocean)
...
if os.path.exists(ofile):
    ...
    PI = potential_intensity(
        ...
        dim_x='longitude', dim_y='latitude', dim_z=dim='isobaricInhPa'
        )
    """
    PI = potential_intensity(
        sst=sst,
...
if os.path.exists(ofile):
    ...
    eta = absolute_vorticity(
        vort850=vort_850.drop(dim='isobaricInhPa'),
        lat=geo_co[0]
    )
    """
...
if os.path.exists(ofile):
    ...
    H = cut_mlp(rh, level_600).drop(dim='isobaricInhPa')
