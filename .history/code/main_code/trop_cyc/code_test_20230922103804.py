...
T_m_6000=cut_mlp(Tm,level_data=level_600)
rh_600=cut_mlp(rh,level_data=level_600)
vort_850=cut_mlp(vort,level_data=le)
...
if os.path.exists(ofile):
    ...
    Tm = cut_mlp(Tm, level_data=level_co)
    RH = cut_mlp(rh, level_data=level_co)
    chi = entropy_deficit(
        sst=sst,
        slp=slp,
        Tb=t2m,
        RHb=rh2m,
        p_m=p_m,
        Tm=cut_mlp(Tm,level_data= level_co).drop('IsobaricInhPa'),
        RHm=cut_mlp(rh,level_data= level_co).drop('IsobaricInhPa')/100
    ).where(is_ocean)
    """
