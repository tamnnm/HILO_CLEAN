...
#
p_test = [200, 500, 850]
z_threshold = [12520, 5850, 5880]
# """
for i, z_p in enumerate(p_test):
    range = 5
    break_factor = 0
    level_co = cut_level(Dataset, level=z_p)
    latlon_co = cut_co(Dataset, dlat=0, ulat=25, dlon=100, ulon=150)
    ...
    for day in np.arange(1, 31):
        ...
        data_test = cut_mlp(Dataset, time_data=time_co,
                            geo_data=latlon_co, lev_data=level_co, dim=['time', 'level'])
        print(data_test)
        data_plot = func_hgt(data_test, range=20)
        print(data_plot)
