...
Tm_600 = cut_mlp(Tm, level_data=level_600).squezze(drop=True)
print(Tm_600.squeeze(drop=True))
...
if os.path.exists(ofile):
    ...
    chi_sb = entropy_deficit(
        ...
        RHb=rh2m,
        p_m=p_m,
        Tm=Tm_600.squeeze(drop=True),
        RHm=rh_600.squeeze(drop=True)/100,
        forGPI2010=True
    ).where(is_ocean)
