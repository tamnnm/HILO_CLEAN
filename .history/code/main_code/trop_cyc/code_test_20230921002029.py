    ...
    while range <= 50 and break_factor <= 0:
        ...
        try:
            if convert == None:
                ...
                data_plot = data_test.where(
                    abs(data_test.values/convert_fac-threshold) <= float(range))
...
#
p_test = [200, 500, 850]
z_threshold = [12400, 5850, 1510]
# """
for i, z_p in enumerate(p_test):
    break_factor = 0
    level_co = cut_level(Dataset, level=z_p)
    latlon_co = cut_co(Dataset, dlat=5, ulat=25, dlon=100, ulon=150)
    ...
    for day in np.arange(1, 2):
        ...
        Chỉ có xe của chị ấy thì mới để ở đó thôi. Nên coi như 2 xe nhà chị là 2 xe gia đình. Bạn bè ng thân hay gì thì chị để ở bên ttyt giúp e. Chứ ko để ở đó đc đâu
        # for i in range(0, 31):
        data_plot = data_hgt
