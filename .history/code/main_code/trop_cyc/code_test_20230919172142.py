...
def func_hgt(data_test, range):
    ...
    while range <= 50 and break_factor <= 0:
        ...
        try:
            dset_plot = data_test.where(
                abs(data_test.values - z_p) <= float(range))
            break_factor = 1
            # print("Success")
...
def dat_clean_plot(dts, opt=None):
    # attribute
    attrs = dts.attrs
    # geodata
    try:
        geo_co = dts.coords[['lat', 'lon']]
    except:
        geo_co = dts.coords[['latitude', 'longitude']]
    # variable
    list_var = dts.variables.keys()
    if list_var > 1:
        print("Please check the number of variables in this case.\n\
        ...
        dts_clean = xr.Dataset(data=dts.variables, coords=co, attrs=attrs)
    else:
        for var in list_var:
            print(f"This dataset has this {var}")
            var = dts.variables[f'{var}']
            # wrap into a new dataarray to avoid multiple dimensions
            dts_clean = xr.DataArray(data=var, coords=geo_co, attrs=attrs)
    return dts_clean
...
for i, z_p in enumerate(p_test):
    ...
    latlon_co = cut_co(Dataset, dlat=5, ulat=25, dlon=100, ulon=118)
    ...
    for day in np.arange(1, 31):
        ...
        data_test = cut_mlp(Dataset, time_data=time_co,
                            geo_data=latlon_co, lev_data=level_co, dim=['time', 'level'])  # even though there is one level butdo this so that the data array is 2 dimension
        # print(data_test)
        # Computation can only be executed with dataarray so change dataset->dataarray
        data_hgt = func_hgt(data_test.to_array(), range=20)
        map_pro = ccrs.PlateCarree()
        fig = plt.figure()
        ax = plt.subplot(111, projection=map_pro)
        # for i in range(0, 31):
        data_plot = dat_clean_plot(data_hgt)
        pl = data_plot.plot.contour(x="longitude", y="latitude", z=data_plot.values,
                                    ax=ax, colors="red", linewidths=2, levels=5)
