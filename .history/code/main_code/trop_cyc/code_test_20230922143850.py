...
for i, z_p in enumerate(p_test):
    ...
    def model_plt(day):
        time_co = cut_time(rh, yr=1964, mon=11, day=int(day+1))
        data_test = cut_mlp(rh, time_data=time_co,
                            geo_data=latlon_c level_data=level_co, dim_mean=['time', 'level'], dim_plot=['geo'])  # even though there is one level butdo this so that the data array is 2 dimension
        ...
        if day == 0:
            pl_hgt = data_test.plot.contourf(
                ax=ax, cmap='RdYlBu', extend="both")  # , linewidths=2, levels=1)
        else:
            pl_hgt = data_test.plot.contourf(
                ax=ax, cmap='RdYlBu', extend="both", add_colorbar=False)
    ...
    anim.save(Data_result+f'sst_animation.gif', writer=writervideo)
