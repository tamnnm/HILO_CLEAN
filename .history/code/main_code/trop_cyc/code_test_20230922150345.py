...
# p_test = [200, 500, 850]
p_test = [700]
z_th = [12400, 5850, 1520]
level_850 = cut_level(uwnd, 850)
level_600 = cut_level(uwnd, 600)
level_200 = cut_level(uwnd, 200)
Tm_600 = cut_mlp(Tm, level_data=level_600).squeeze(drop=True)
rh_600 = cut_mlp(rh, level_data=level_600).squeeze(drop=True)
vort_850 = cut_mlp(vort, level_data=level_850).squeeze(drop=True)
u_200=cut_mlp(uwnd, level_data=level_200).squeeze(drop=True)
v_200=cut_mlp(vwnd, level_data=level_200).squeeze(drop=True)
u_850=cut_mlp(uwnd, level_data=level_850).squeeze(drop=True)
v_850=cut_mlp(vwnd, level_data=level_850).squeeze(drop=True)
V_shear=( (u_200-u_850)**2 + (v_200-v_850)**2 )**0.5
...
for i, z_p in enumerate(p_test):
    ...
    def model_plt(day):
        time_co = cut_time(V_shear, yr=1964, mon=11, day=int(day+1))
        data_test = cut_mlp(V_shear, time_data=time_co,
                            geo_data=latlon_co, dim_mean=['time','lat'], dim_plot=['lon','level']).where(is_ocean)
        print(data_test)
        ...
        # data_hgt = func_hgt(data_test=data_test, threshold=z_threshold,
        #                    range=5, convert="yes")
        # print(data_test)
        if day == 0:
            pl_hgt = data_test.plot.contourf(
                ax=ax, cmap='coolwarm', extend="both")  # , linewidths=2, levels=1)
        else:
            pl_hgt = data_test.plot.contourf(
                ax=ax, cmap='coolwarm', extend="both", add_colorbar=False)
        # pl_hgt = data_hgt.plot.contour(
    ...
    def animate(day):
        ...
        plt.title(f'{day}-11/1964', pad=25)
    ...
    anim.save(Data_result+f'Vshear_{z_p}_animation.gif', writer=writervideo)

    # Extracct level from gribfile
    """
    for day in np.arange(1, 2):
        time_co = cut_time(hgt, yr=1964, mon=11, day=day)
        data_test = cut_mlp(hgt, time_data=time_co,
...
ofile = os.path.join(Data_result, f'.PI.nc')
if os.path.exists(ofile):
    PI = xr.open_dataset(ofile)
    print('[opened]:', ofile)
else:
    def reverse_plevels(x): return x.isel(isobaricInhPa=slice(-1, None, -1))
    """
    ...
    PI = potential_intensity(
        sst=sst,
        slp=slp.where(is_ocean),
        p=p_lev.pipe(reverse_plevels)*100,
