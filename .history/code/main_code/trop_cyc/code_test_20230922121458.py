...
for i, z_p in enumerate(p_test):
    ...
    for day in np.arange(1, 2):
        time_co = cut_time(hgt, yr=1964, mon=11, day=day)
        data_test = cut_mlp(hgt, time_data=time_co,
                            geo_data=latlon_co, level_data=level_co, dim_mean=['time' 'isobaric], dim_plot=['geo'])  # even though there is one level butdo this so that the data array is 2 dimension
        # print(data_test.values)
        # Computation can only be executed with dataarray so change dataset->dataarray
        data_hgt = func_hgt(data_test=data_test, threshold=z_threshold[i],
                            range=5, convert="yes")
