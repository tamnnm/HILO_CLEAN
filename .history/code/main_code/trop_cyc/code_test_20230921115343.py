...
def set_up_plot(dlat, ulat, dlon, ulon, pad):
    ...
    lon_formatter = LongitudeFormatter(zero_direction_lable=False)
    lat_formatter = LatitudeFormatter(zero_direction_lable=False)
...
for i, z_p in enumerate(p_test):
    ...
    for day in np.arange(1, 2):
        ...
        (fig, ax) = set_up_plot(-5, 25, 80, 150, 5)
        for i in np.arange(1, 2):
            data_plot = data_hgt
        # print(data_hgt.values)
            pl = data_plot.plot.contour(
                ax=ax, colors="r", linewidths=2, levels=5)
        # Set
