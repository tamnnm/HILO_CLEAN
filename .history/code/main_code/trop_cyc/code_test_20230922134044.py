...
for i, z_p in enumerate(p_test):
    ...
    def model_plt(day):
        time_co = cut_time(hgt, yr=1964, mon=11, day=int(day+1))
    ...
    def animate(day):
        ...
        plt.title(f'{z_p}-11/1964', pad=25)
        ...
        # plt.close()
        return pl_hgt
    anim = FuncAnimation(fig, animate, frames=Nt, interval=30, repeat=True,blit=True)
    # , writer=FFMpegWriter())
    writervideo = PillowWriter(fps=60)
    anim.save(Data_result+f'{z_p}_animation.gif', writer=writervideo)
