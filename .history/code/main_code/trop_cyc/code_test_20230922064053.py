...
path_grib = "/work/users/tamnnm/Data/wrf_data/gribfile/era5_6h_1940_2023/"
result = "/work/users/tamnnm/Data/wrf_data/gribfile/sub_result/"
def call_dataset(filename):
    if os.path.exists(filename):
        Dataset = xr.open_dataset(f'{path_grib+filename}',
                          engine="cfgrib")
        return Dataset
    else:
...
year_org=1964
hgt=call_dataset(f"geopotential_1964_b_even.grib")
...
T=call_dataset('temperature_1964_b_even.grib')
rh2m = (100*(np.exp(17.625*td2m/(243.04 + td2m))/np.exp(17.625 *t2m/(243.04 + t2m))))/100,
...
# ----------------------- file in wrf_output ----------------------- #
"""
#path_wrf = "/work/users/tamnnm/wrf/WRF/test/RUN_ERA5_196411_9/"
# for filename in (os.listdir(f'{path}/wrfout*')):
...
dname = 'chi'
ofile = os.path.join(result, f'.{dname}.nc' )
if os.path.exists(ofile):
    ...
    Tm=cut_mlp(T,level_data=level_co)
    RH=cut_time(rh,level_data=level_co)
    chi = entropy_deficit(
        sst=sst,
        slp=slp,
        Tb=t2m,
        RHb=rh2m,
        p_m=p_m,
        Tm=T.sel(level=p_m/100).drop('IsobaricInhPa'),
        RHm=rh.sel(level=p_m/100).drop('IsobaricInhPa')/100
        ).where(is_ocean)
    """
...
dname = 'chi_sb'
ofile = os.path.join(result, ('.nc', f'.{dname}.nc') )
if os.path.exists(ofile):
...
ofile = os.path.join(result, ('.nc', f'.PI.nc') )
if os.path.exists(ofile):
    ...
    PI = potential_intensity(
        sst=sst,
        slp=slp.where(is_ocean),
        p=T.level.pipe(reverse_plevels)*100,
        T=T.pipe(reverse_plevels).where(is_ocean),
...
if __name__ == '__main__':
    #year = 1979
    #do_tci(year, result='/tigress/wenchang/data/era5/analysis/TCI/')
    #years = range(1979, 1979+1)
    result = '/tigress/wenchang/data/era5/analysis/TCI_v202201/'
    if len(sys.argv)>1:
        year = int(sys.argv[1])
        do_tci(year, result=result)
    else:
