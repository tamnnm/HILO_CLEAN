...
Data_Arr = xr.open_dataset(f'{Data_path_grib+filename}',
                          engine="cfgrib")
...
for i, z_p in enumerate(p_test):
    ...
    for day in np.arange(1, 31):
        time_co = cut_time(Data_Arr, yr=1964, mon=11, day=day)
        data_test = cut_mlp(Data_Arr, time_data=time_co,
                            geo_data=latlon_co, lev_data=level_co, ax=0)
