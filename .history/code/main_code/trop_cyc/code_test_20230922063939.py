...
year_org=1964
Dataset_hgt=call_dataset(f"geopotential_1964_b_even.grib")
Dataset_sst=call_dataset("sea_surface_temperature_1964_even.grib")
Dataset_t2m=call_dataset('2m_temperature_1964_even.grib')
Dataset_td2m=call_dataset('2m_dewpoint_temperature_1964_even.grib')
Dataset_T=call_dataset('temperature_1964_b_even.grib')
Dataset_rh2m = (100*(np.exp(17.625*Dataset_td2m/(243.04 + Dataset_td2m))/np.exp(17.625 *Dataset_t2m/(243.04 + Dataset_t2m))))/100,
Dataset_rh= call_dataset('relative_humidity_1964_b_even.grib')
Dataset_sh=call_dataset('specific_humidity_1964_b_even.grib')
Dataset_slp=call_dataset('mean_sea_level_pressure_1964_even.grib')
land_mask_sea=call_dataset('land_sea_mask_1964_even.grib')
is_ocean=Dataset_sst.isel(time=0).drop('time').pipe(lambda x: x*0==0)
Data_uwnd=call_dataset('u_component_of_wind_1964_b_even.grib')
Data_vwnd=call_dataset('v_component_of_wind_1964_b_even.grib')
# ----------------------- file in wrf_output ----------------------- #
"""
...
for i, z_p in enumerate(p_test):
    ...
    for day in np.arange(1, 2):
        ...
        data_test = cut_mlp(Dataset, time_data=time_co,
                            geo_data=latlon_co, level_data=level_co, dim_mean=['time', 'IsobaricInhPa'], dim_plot=['geo'])  # even though there is one level butdo this so that the data array is 2 dimension
...
if os.path.exists(ofile):
    ...
    p_m = 600*100 # Pa
    level_co=cut_level(Dataset,level=p_m/100)
    Tm=cut_mlp(Dataset_T,level_data=level_co)
    RH=cut_time(Dataset_rh,level_data=level_co)
    chi = entropy_deficit(
        sst=Dataset_sst,
        slp=Dataset_slp,
        Tb=Dataset_t2m,
        RHb=Dataset_rh2m,
        p_m=p_m,
        Tm=Dataset_T.sel(level=p_m/100).drop('IsobaricInhPa'),
        RHm=Dataset_rh.sel(level=p_m/100).drop('IsobaricInhPa')/100
        ).where(is_ocean)
    """
...
if os.path.exists(ofile):
    ...
    PI = potential_intensity(
        sst=Dataset_sst,
        slp=Dataset_slp.where(is_ocean),
        p=Dataset_T.level.pipe(reverse_plevels)*100,
        T=Dataset_T.pipe(reverse_plevels).where(is_ocean),
        q=Dataset_sh.pipe(reverse_plevels).where(is_ocean),
        dim_z='IsobaricInhPa'
        )
    """
    encoding = {dname:{'dtype': 'float32', 'zlib': True, 'complevel': 1}
...
if os.path.exists(ofile):
    ...
    level_850=cut_level(Data_uwnd,850)
    level_200=cut_level(Data_uwnd,200)
    Vshear = wind_shear(
        u850=Data_uwnd.sel(level=850),
        v850=Data_vwnd.sel(level=850),
        u200=Data_v.sel(level=200),
...
if os.path.exists(ofile):
    ...
    eta = absolute_vorticity(
        vort850=vort.sel(level=850).drop('IsobaricInhPa'),
        lat=vort.latitude
        )
...
if os.path.exists(ofile):
    ...
    H = RH.sel(level=600).drop('IsobaricInhPa')
...
if __name__ == '__main__':
    #year = 1979
    #do_tci(year, Data_result='/tigress/wenchang/data/era5/analysis/TCI/')
    #years = range(1979, 1979+1)
    Data_result = '/tigress/wenchang/data/era5/analysis/TCI_v202201/'
    if len(sys.argv)>1:
        year = int(sys.argv[1])
        do_tci(year, Data_result=Data_result)
    else:
