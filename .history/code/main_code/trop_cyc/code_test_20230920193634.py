...
def func_hgt(data_test, range, convert=None):
    convert_fac=9.80665
    ...
    if convert==None:
        ...
        dset_plot = data_test.where(
                abs(data_test.values/convert_fac - z_p) <= float(range))
...
for i, z_p in enumerate(p_test):
    ...
    for day in np.arange(1, 2):
        ...
        data_hgt = func_hgt(data_test, range=20)
        ...
        pl = data_plot.plot.contour(
            ax=ax, colors="red", linewidths=2, levels=5, masked=True)
        ...
        plt.savefig(os.path.abspath(
            rf'/work/users/tamnnm/code/main_code/trop_cyc/img/era5_{test[i]}hPa_{day}_11.jpg'), format="jpg", dpi=300)
        plt.close()
