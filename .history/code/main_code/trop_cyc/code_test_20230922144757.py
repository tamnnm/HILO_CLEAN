...
for i, z_p in enumerate(p_test):
    ...
    def model_plt(day):
        time_co = cut_time(sst, yr=1964, mon=11, day=int(day+1))
        data_test = cut_mlp(sst, time_data=time_co,
                            geo_data=latlon_co, dim_mean=['time'], dim_plot=['geo']).where(is_ocean) - 273.5
        print(data_test)
    ...
    anim.save(Data_result+f'sst_{z_p}_animation.gif', writer=writervideo)
