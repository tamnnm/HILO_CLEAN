...
def func_hgt(data_test, lv, range, convert=None):
    convert_fac = 9.80616
        ...
        try:
            if convert == None:
                data_plot = data_test.where(
                    abs(data_test.values - lv) <= float(range))
            else:
                data_plot = data_test.where(
                    abs(data_test.values/convert_fac-lv) <= float(range))
                # print(data_test.values/convert_fact)
                print(abs(data_test.values/convert_fac-lv))
            break_factor = 1
            # print("Success")
