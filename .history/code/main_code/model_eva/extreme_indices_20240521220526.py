...
def run_cdo_command(command: Callable, input: str, output: str, args: Optional[List] = None, check=True):
    if args is None:
        args = {}
    if check:
...
class gen_ind:
    ...
    def __call__(self, ind, dataset):
        ...
        print(self.command, self.ifile, self.ofile, '\n')
        # self.ifile = None
...
args = [(ind, dataset) for ind in rain_tuple for dataset in list(rean_year_dict.keys())]
...
if base_status:
    print("You must check the base file first")
