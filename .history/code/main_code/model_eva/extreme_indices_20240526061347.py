...
metrics_compare=['per_error','R','nrmse','nmae','DISO','Taylor_score']
...
def merge_diso_df(var,metrics):
    ...
    def gen_diso_df(dataset):
        ...
        ind_ind = f'{Data_csv}{dataset}_{var}.csv'
        ...
        for ind in list_ind:
            ...
            for i, no_city in enumerate(list_no_city):
                ...
                if i==0:
                    result_df=[evaluate_all(obs_data, rean_data,metrics_compare)]
                else:
                    result_df.append(evaluate_all(obs_data, rean_data,metrics_compare))

            # Example at this step: result_df = [{'CDD': 1,'CWD':2},{'CDD': 3,'CWD':4},{'CDD': 5,'CWD':6]
            result_df =  {key: [d.get(key, None) for d in result_df] # Get the value of the key and put in list
                          for key in set().union(*result_df)} # Get the set of all the keys

            # Example at this step: result_df = {'CDD': [1,2,3,4,5,7],'CWD':[...]}
    ...
    merge_file_name = f'{Data_csv}{var}_diso_merge.csv'
    ...
    if check_non_exist(merge_file_name):
        ...
        for i, rean_name in enumerate(list(rean_year_dict.keys())):
            ...
            if check_non_exist(f"{Data_csv}{rean_name}_{var}.csv"):
                df = gen_diso_df(rean_name)
            else:
                df = pd.read_csv(
                    f"{Data_csv}{rean_name}_{var}.csv")
            ...
            if i == 0:
                ...
                merge_df = merge_df.merge(
                    df, on=['name_station', 'ind'], how='outer')
    ...
    df_piv.to_csv(Data_csv+var+"_diso_pivot.csv")
    return df_piv
...
def plot_tri(var):
    ...
    if check_non_exist(Data_csv+"_diso_pivot.csv"):
        df_piv = merge_diso_df(var=var)
    else:
        df_piv = pd.DataFrame(Data_csv+"_diso_pivot.csv")
