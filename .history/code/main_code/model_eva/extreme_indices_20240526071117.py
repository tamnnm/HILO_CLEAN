    ...
    def gen_diso_df(dataset):
        ...
        for ind in list_ind:
            ...
            for i, no_city in enumerate(list_no_city):
                ...
                if i==0:
                    result_subdict=[evaluate_all(obs_data, rean_data,metrics_compare)]
                else:
                    result_subdict.append(evaluate_all(obs_data, rean_data,metrics_compare))

            try:
            # Example at this step: result_df = [{'R': 1,'DISO':2},{'R': 3,'DISO':4},{'R': 5,'DISO':6]
                result_dict =  {key: [d.get(key, None) for d in result_subdict] # Get the value of the key and put in list
                          for key in set().union(*result_subdict)} # Get the set of all the keys

            # Example at this step: result_df = {'R': [1,3,5],'DISO':[...]}

                result_df = pd.DataFrame({'name_station': list_name_city,
                                    'ind': [ind]*len(list_name_city),
                                    **result_dict})
            except Exception as e:
                print(ind, dataset)
