...
metrics_compare = ['mpe','mape', 'R', 'nme', 'nmae', 'nrmse', 'DISO', 'Taylor_score']
        ...
        if check_non_exist(pivot_file_name):
            ...
            # Initiate the merge dataframe
                for i, rean_name in enumerate(list(rean_year_dict.keys())):
                    ...
                    merge_sub_df = dfs_merge[rean_name][['name_station', 'ind', metric]].rename({metric: rean_name}, axis=1)
...
def plot_tri(var):
    ...
    print(len(np.ravel(val)))
    print(len(triangul.triangles))
