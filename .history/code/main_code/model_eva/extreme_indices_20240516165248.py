...
def merge_diso_df(var):
    ...
    def gen_diso_df(dataset):
        ...
        for ind in list_ind:
            ...
            [DISO(cut_mlp(dts=cut_points(sub_rean, pts_dict[list_name_city[i]], ['lat', 'lon'], full=True)['full'], dim_mean=['geo']),sub_obs.where(sub_obs['no_station'] == no_city, drop=True)) for i, no_city in enumerate(list_no_city)]
