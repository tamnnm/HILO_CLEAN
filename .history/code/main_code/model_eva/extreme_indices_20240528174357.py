...
def plot_tri(var, metric='DISO'):
    ...
    cb = fig.colorbar(imgs[0], ax=ax,
                      extend='both', extendfrac='auto')
    tick_locator = ticker.MaxNLocator(nbins=9)
    cb.locator = tick_locator
    cb.update_ticks()
    # Left, bottom, width, height
    xy_inset=[[0,0],[0,2],[2,0],[2,2],[1,1]]
    triangles_inset=[[0,1,4],[0,2,4],[1,3,4],[]]
    ins = ax.inset_axes([0.1, 1.1, 0.15, 0.2])
    ins.tripcolor(Triangulation([ ], [0, 1, 2], [[0, 1, 2]]), [
                  0, 1, 2], cmap=my_cmap[0], norm=norms[0])
