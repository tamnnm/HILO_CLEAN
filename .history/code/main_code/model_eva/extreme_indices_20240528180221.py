...
def merge_df(var, metric_stop=None):
    ...
    def gen_df(dataset):
        ...
        df_all.to_csv(ind_csv, index=False)
...
def plot_tri(var, metric='DISO'):
    ...
    if check_non_exist(Data_csv+f"ensemble/{var}_{metric}_pivot.csv"):
        ...
        df_piv = pd.DataFrame(Data_csv+f"ensemble/{var}_{metric}_pivot.csv")
    ...
    ax.set_xticklabels(df_piv['cera'].columns, rotation=90, fontsize=10)
    ax.set_yticks(range(N))
    ax.set_yticklabels(df_piv.index, fontsize=12)
    ax.invert_yaxis()
    ax.margins(x=0, y=0)
    ...
    cb = fig.colorbar(imgs[0], ax=ax, pad=-0.5,
    ...
    ins = ax.inset_axes([0.9, 1.1, 0.07, 0.2])
    ...
    ins.set_xlabel('ERA5')
    # labels are at the bottom by default
    ins.xaxis.set_label_position('bottom')
    ins.set_ylabel('ERA')
    ins.yaxis.set_label_position('left')  # labels are on the left by default

    # Add labels to top and right of inset axes
    ins.set_xlabel('CERA', top=True)
    ins.xaxis.set_label_position('top')
    ins.set_ylabel('20CR', right=True)
    ins.yaxis.set_label_position('right')

    plt.suptitle(f"{metric} Evaluation for {var} indices")
    fig.tight_layout()
    fig.savefig(img_wd+f"{var}_{metric}_triheatmap.png", format="png")
    return
