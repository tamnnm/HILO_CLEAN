...
def merge_df(var):
    ...
    def gen_df(dataset):
        ...
        obs_ind = f'{Data_csv}obs_{dataset}_{var}.csv'
...
# ------------------------- Start to merge ------------------------- #
    ...
    for metric in metrics_compare+metrics_singular:
        ...
        merge_file_name = f'{Data_csv}{var}_{metric}_merge.csv'
        ...
        # Initiate the merge dataframe
            for i, rean_name in enumerate(list(rean_year_dict.keys())):
                ...
                merge_sub_df = dfs_merge[rean_name].loc[['name_station', 'ind', metric]]

                # This can may flood the RAM but keep name_station and ind
                if i == 0:
                    merge_df = dfs_merge[rean_name][['name_station', 'ind', metric]]
                else:
                    # Join along these two collumns to align the reanalysis with each other
                    # Keep in mind the order of the reanlysis datasets
                    merge_df = merge_df.merge(df[[metric]], how='outer')
