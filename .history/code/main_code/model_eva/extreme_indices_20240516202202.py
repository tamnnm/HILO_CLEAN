        ...
        for ind in list_ind:
            ...
            diso_df = pd.DataFrame({'name_station': list_name_city,
                                   'ind': [ind]*len(list_name_city),
                                    ...
                                    dataset: [DISO(sub_obs.where(sub_obs['no_station'] == no_city, drop=True)cut_mlp(dts=cut_points(sub_rean, pts_dict[list_name_city[i]], ['lat', 'lon'], full=True)['full'], dim_mean=['geo']),
