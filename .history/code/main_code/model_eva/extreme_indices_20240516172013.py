...
indice_dict = {167: {'min': ('Tnn', 'Tnx', 'Tn10p', 'Tn90p', 'CSDI', 'TR20',), 'max': ('Txx', 'Txn', 'Tx90p', 'Tx10p', 'WSDI', 'SU25',), 'other': ('DTR',)},
...
rean_year_dict = {'cera': {'start_year': 1901, 'end_year': 2010},
...
def check_non_exist(name):
    return False if os.path.exists(name) else True

# ------------------------- Run cdo command ------------------------ #


def run_cdo_command(command: Callable, input: str, output: str, args: Optional[List] = None):
    if args is None:
        args = {}
...
def gen_base(var: Optional[Literal["tp", "max", "min", "vwnd", "uwnd"]], dataset: Optional[Literal["cera", "era", "era5", "noaa", "obs"]],
             start: Optional[int] = 1961, end: Optional[int] = 1990, percentile: list = None, window='5', *args, **kwargs):
    # create base file
    ...
    if check_non_exist(full_ofile):
        ...
        if no == 228:
            cdo.seldate(
                time_range, input=f'-setctomiss,0 -mul {full_ifile} -gec,1 {full_ifile}', output=full_ofile, options='-L')
            print("Special ", full_ofile)
        else:
            cdo.seldate(time_range, input=full_ifile,
                        output=full_ofile, options='-L')
            print(full_ofile)
    ...
    if param_dict[var]['type'] == 'extreme':
        ...
        if percentile:
            ...
            # Minimum values
            run_cdo_command(cdo.ydrunmin, ibfile, pminfile, [window])
            # Maximum values
            run_cdo_command(cdo.ydrunmin, ibfile, pmaxfile, [window])
            ...
            for rp in percentile:
                ...
                # Quantile file
                run_cdo_command(
                    cdo.selyear, f'-ydrunpctl,{rp},{window},pm=r8 {ibfile} {pminfile} {pmaxfile}', opfile, [f'{start}/{end}',])
    return
...
class gen_ind:
    ...
    def __call__(self, ind, dataset):
        ...
        self.imaxfile = self.fol+iminfile
...
# Done
# -------------------------- RUN THE TEST -------------------------- #
# Run this after you have run the upper part
# region
...
# Observation
# for ind in rain_tuple:
#     gen_class(ind, 'obs')
# for ind in temp_tuple:
#     gen_class(ind, 'obs')
...
def merge_diso_df(var):
    ...
    list_name_city = sel_ds['name_station'].values
    print(all(isinstance(item, bytes) for item in sel_ds['name_station'].values))
    raise KeyboardInterrupt
    # Generate the diso index for each reaannalysis dataset

    def gen_diso_df(dataset):
        # define name of file of indices
        ind_ind = f'{Data_ind}{dataset}_{var}_indices.csv'
        if not check_non_exist(ind_ind):
            return pd.read_csv(ind_ind)
        ...
        def subset_year(file_name, var_tup, mode="obs"):
            ...
            # drop = True to remove the NaN values
            # Beware below we don't use drop=True
        diso_df_list = []
        for ind in list_ind:
            ...
            # print(sub_obs)
            # print(len(sub_obs.where(sub_obs['no_station'] == 0, drop=True).shape))
            # print(DISO(cut_mlp(dts=cut_points(sub_rean, pts_dict['ALUOI'], [
            #       'lat', 'lon'], full=True)['full'], dim_mean=['geo']), sub_obs.where(sub_obs['no_station'] == 0, drop=True)))
            print(list_name_city)
            print(list_no_city)
            print(pts_dict['BACHLONGVI'])
            test = [DISO(cut_mlp(dts=cut_points(sub_rean, pts_dict[list_name_city[i]], ['lat', 'lon'], full=True)['full'], dim_mean=[
                         'geo']), sub_obs.where(sub_obs['no_station'] == no_city, drop=True)) for i, no_city in enumerate(list_no_city)]
            raise KeyboardInterrupt
            diso_df = pd.DataFrame({'name_station': list_name_city,
                                   'ind': [ind]*len(list_name_city),
                                    ...
                                    dataset: [DISO(cut_mlp(dts=cut_points(sub_rean, pts_dict[list_name_city[i]], ['lat', 'lon'], full=True)['full'], dim_mean=['geo']),
                                                   sub_obs.where(sub_obs['no_station'] == no_city, drop=True)) for i, no_city in enumerate(list_no_city)]})
        ...
        diso_df_all = pd.concat(diso_df_list, ignore_index=True)
        diso_df_all.to_csv(ind_ind)
        return diso_df_all

    # Merge the indice dataframe

    # define of the merge file name
    merge_file_name = f'{Data_ind}{var}_merge_indices.csv'
    ...
    if check_non_exist(merge_file_name):
        ...
        for i, rean_name in enumerate(list(rean_year_dict.keys())):
            ...
            if check_non_exist(f"{Data_ind}{rean_name}_{var}_indices.csv"):
                df = gen_diso_df(rean_name)
            else:
                df = pd.read_csv(
                    f"{Data_ind}{rean_name}_{var}_indices.csv")
            ...
            if i == 0:
                ...
                merge_df.join(df, on=['name_station', 'ind'], how='outer')
        ...
        merge_diso_df.to_csv(merge_file_name)
    else:
    ...
    df_piv.to_csv(Data_ind+var+"_pivot_indices.csv")
    return df_piv
...
def plot_tri(var):
    ...
    if check_non_exist(Data_ind+"pivot_indices.csv"):
        df_piv = merge_diso_df(var=var)
    else:
        df_piv = pd.DataFrame(Data_ind+"pivot_indices.csv")
