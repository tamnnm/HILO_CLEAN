    ...
    for metric in metrics_compare+metrics_singular:
        ...
        # Initiate the merge dataframe
            for i, rean_name in enumerate(list(rean_year_dict.keys())):
                # Take the subset of the dataframe and rename the
                merge_sub_df = dfs_merge[rean_name].loc[['name_station', 'ind', metric]].rename({metric: rean_name}, axis=1)
                ...
                if i == 0:
                    ...
                    merge_df = merge_df.merge(merge_sub_df, on=['name_station', 'ind'], how='outer')
