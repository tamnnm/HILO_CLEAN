...
# ------------------------- Start to merge ------------------------- #
    ...
    for metric in metrics_compare+metrics_singular:
        ...
        df_return = df_piv if metric_stop is not None else None
    return df_return
...
def plot_tri(var):
    ...
    if check_non_exist(Data_csv+f"ensemble/{var}_diso_pivot.csv"):
        df_piv = merge_df(var=var, metric_stop='DISO')
    else:
        df_piv = pd.DataFrame(Data_csv+f"ensemble/{var}_diso_pivot.csv")
    ...
    print(len(values), len(triangul))

    my_cmap = ['RdYlBu'] * no_rean
    # my_cmap.set_bad('#FFFFFF00')
    # my_cmap.set_under('#FFFFFF00')
    #norms = [plt.Normalize(vmin=0, vmax=4) for _ in range(no_rean)]
    ...
    imgs = [ax.tripcolor(t, np.ravel(val), cmap=cmap, ec='black')
            for t, val, cmap, norm in zip(triangul, values, my_cmap)]
    ...
    plt.colorbar(imgs[0], ax=ax, shrink=0.6)
    plt.tight_layout()
