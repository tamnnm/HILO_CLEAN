...
def run_cdo_base(command: str, op: None, input: str, output: str, args: Optional[List] = None):
    if op is None:
        cdo_command = [
            'cdo', '-L', f'{command}',
            input, output
    ...
    result = subprocess.run(cdo_command, check=True)
    return result
...
def gen_base(var: Optional[Literal["tp", "max", "min", "vwnd", "uwnd"]], dataset: Optional[Literal["cera", "era", "era5", "noaa", "obs"]],
    ...
    if param_dict[var]['type'] == 'extreme':
        ...
        if percentile:
            ...
            # Maximum values
            run_cdo_command(cdo.ydrunmax, ibfile, pmaxfile, [window])
            # n-window running percentile of base value
            for rp in percentile:
                opfile = full_name(f'b{rp}p_{ifile}')
                # Quantile file
                cdo.ydrunpctl(
                    f'{rp},{window},pm=r8', input=f'{ibfile} {pminfile} {pmaxfile}', output=opfile, option='-L')
                print(opfile)
    return
...
def merge_diso_df(var):
    ...
    def gen_diso_df(dataset):
        ...
        ind_ind = f'{Data_csv}{dataset}_{var}_indices.csv'
        ...
        for ind in list_ind:
            ...
            diso_df = pd.DataFrame({'name_station': list_name_city,
                                   'ind': [ind]*len(list_name_city),
                                    ...
                                    dataset: [print(no_city) or DISO(sub_obs.where(sub_obs['no_station'] == no_city, drop=True),
    ...
    merge_file_name = f'{Data_csv}{var}_merge_indices.csv'
    ...
    if check_non_exist(merge_file_name):
        ...
        for i, rean_name in enumerate(list(rean_year_dict.keys())):
            ...
            if check_non_exist(f"{Data_csv}{rean_name}_{var}_indices.csv"):
                df = gen_diso_df(rean_name)
            else:
                df = pd.read_csv(
                    f"{Data_csv}{rean_name}_{var}_indices.csv")
    ...
    df_piv.to_csv(Data_csv+var+"_pivot_indices.csv")
    return df_piv
...
def plot_tri(var):
    ...
    if check_non_exist(Data_csv+"pivot_indices.csv"):
        df_piv = merge_diso_df(var=var)
    else:
        df_piv = pd.DataFrame(Data_csv+"pivot_indices.csv")
