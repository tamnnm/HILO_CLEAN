...
def merge_diso_df(var):
    ...
    def gen_diso_df(dataset):
        ...
        """
        start_year = max(rean_year_dict[dataset]['start_year'], 1961)
        ...
        def subset_year(file_name, var_tup, mode="obs"):
            ...
            with xr.open_dataset(file_name) as full_ds:
                ...
                print(ds.time)
            return ds.where((ds.time.dt.year >= start_year) & (ds.time.dt.year <= end_year)), main_var_name
        ...
        for ind in list_ind:
            ...
            (sub_rean, var_rean), (sub_obs, var_obs) = subset_year(
                rean_ind, param_dict[var], mode='rean'), subset_year(oebs_ind, var)

            # print(cut_mlp(dts=cut_points(sub_rean, pts_dict['ALUOI'], [
            #       'lat', 'lon'], full=True)['full'], dim_mean=['geo']))
            # print(sub_obs.where(sub_obs['no_station'] == 0, drop=True))
