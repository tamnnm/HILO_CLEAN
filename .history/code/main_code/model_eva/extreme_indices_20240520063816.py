...
def run_cdo_base(command: str, args:None, input: str, output: str):
    if args is None:
        cdo_command = [
            'cdo', '-L', f'{command}',
            input, output
...
def gen_base(var: Optional[Literal["tp", "max", "min", "vwnd", "uwnd"]], dataset: Optional[Literal["cera", "era", "era5", "noaa", "obs"]],
    ...
    if check_non_exist(full_ofile):
        ...
        if no == 228:
            run_cdo_base(
                cdo.seldate, args=time_range, input=f'-setctomiss,0 -mul {full_ifile} -gec,1 {full_ifile}', output=full_ofile, options='-L')
    ...
    if param_dict[var]['type'] == 'extreme':
        ...
        if percentile:
            ...
            # Minimum values
            run_cdo_base('ydrunmin', args=window, input=ibfile, output=pminfile)
            # Maximum values
            run_cdo_base('ydrunmax', args=window, input=ibfile, output=pmaxfile)
            # n-window running percentile of base value
            for rp in percentile:
                opfile = full_name(f'b{rp}p_{ifile}')
                # Quantile file
                run_cdo_command(
                    cdo.ydrunpctl, input=f'{ibfile} {pminfile} {pmaxfile}', output=opfile, args=[rp,window,'pm=r8'])
                print(opfile)
    return
