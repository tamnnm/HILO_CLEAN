...
for file_path in Data_name.keys():
    if file_path == 'era5':
        file_name = "indice/mean_era5_167_2000.nc"
    else:
        file_name = f"subset/mean_{file_path}_167.nc"
    with xr.open_dataset(Data_path+Data_name[file_path]+file_name) as ds:
        try:
    ...
    # MUST: add values or else it appears as a dataarray subset
        reso = lat_all[1]-lat_all[0]

        with open(Code_wd+file_path+"euler_pts.txt", 'r') as f:
            lines = f.readlines()
            f.close()
        city_name = []
        for i, line in enumerate(lines):
            city_name.append(line.strip().split(',')[0])
        ...
        for city_list in glob.glob(Code_wd+"*.txt"):
            ...
            for i, line in enumerate(lines[1:]):
                ...
                for lat_test in lat_all:
                    if abs(lat_test-lat_target) > reso:
                        continue
                    else:
            ...
            with open(Code_wd+file_path+"euler_pts.txt", 'w+') as f:
