...
def gen_base(ofile,start,end,dataset,var):
    #create base file for wet days
    org_name=f'para_{var}/{dataset}_{var}'
    ifile=f'{org_name}.nc'
    ofile,minfile,maxfile,p99file,p95file,p90file,p10file=f'n_{ifile}',f'x_{ifile}',f'p99_{ofile}',f'p95_{ofile}',f'p90_{ofile}',f'p10_{ofile}'
    if os.path.exist(ofile):
        ...
        if var in param['extreme'].keys():
           if var=="tp":
                cdo.timmin(input=ifile,output=minfile) if check_exist(minfile)
                cdo.timmax(input=ifile,output=maxfile) if check_exist(maxfile)
                cdo.timpctl('99',input=ofile,output=p99file)
           cdo.timpctl()
