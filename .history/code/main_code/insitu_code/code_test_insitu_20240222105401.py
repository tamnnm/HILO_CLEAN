...
def gen_base(var:Optional[Literal[list(param.keys())]],start:Optional[int],end:Optional[int],dataset:Optional[Literal["cera","era","era5","noaa","obs"]],percentile:list = None,window='5'):
    #create base file
    var_no=param[var]
    fol=f'para_{var_no}/'
...
def get_name_files(ind, dataset: Optional[Literal["cera","era","era5","noaa", "obs"]],type_ind: Optional[Literal['']] =None, base: bool=False, percentile: Optional[str] = None, window: Optional[int] = 5):
    ...
    def get_name(fol,dataset,var_no,type_ind=None, base: bool=False, percentile: Optional[str] = None):
        ifile= fol + f'{dataset}_{var_no}.nc' if type_ind is None else fol + f'{var}{dataset}_{var_no}.nc'
        if base is True:
