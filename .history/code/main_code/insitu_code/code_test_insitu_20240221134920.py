...
class gen_ind:
    ...
    def gen_compare_pctlday(self):
        ...
        if not check_exist(self.ofile): cdo.yearsum(input=f'-ltc,0 -sub {self.bfile} {self.ifile}',output=self.ofile)
            if not check_exist(elf.ofile): cdo.yearsum(input=f'-gtc,0 -sub {self.bfile} {self.ifile}',output=self.ofile)
    def gen_min(self):
    ...
    def gen_wdsi(self):
        cdo.year(input=f'-consects -,1 {self.ifile}',output=self.ofile)
        if self.ind=="wdsi":
            if not check_exist(self.ofile): cdo.yearsum(input=f'-lc,0 -sub {self.bfile} {self.ifile}',output=self.ofile)
        else:
