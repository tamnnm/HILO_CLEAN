        ...
        if var in param['extreme'].keys():
            if percentile:
                ...
                for rp in percentile:
                    ...
                    if check_exist(pfile): cdo.timpctl(str(rp),input=f'{ibfile} -timin {ibfile} -timax{ibfile}',output=pfile)
    return

class precip_ind(object):
    def __call__(self,ind,dataset):
        ind=self.indice
        dataset=self.dataset
        if ind == "cdd":
            self.gen_cdd(dataset)
    fol="para_228"
    def gen_cdd(dataset):
        cdo.yearmax(input='-consect -gtc,0 -yearmax -selyear,1961/2019 '+dataset+'_228.nc',output=self.fol+'/cdd_'+dataset+'_228.nc')
