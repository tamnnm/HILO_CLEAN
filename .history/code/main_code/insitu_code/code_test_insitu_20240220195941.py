...
class precip_ind:
    ...
    def gen_cwd(self):
        if not check_exist(self.ofile):
            cdo.yearmax(input=f'-consects -gtc,1 {self.ifile}',output=self.ofile)
    def gen_r10mm(self):
        if not check_exist(self.ofile):
            cdo.yearsum(input=f'-gtc,10 {self.ifile}',output=self.ofile)
    def gen_r20mm(self):
        if not check_exist(self.ofile):
            cdo.yearsum(input=f'-gtc,20 {self.ifile}',output=self.ofile)
    def gen_r50mm(self):
