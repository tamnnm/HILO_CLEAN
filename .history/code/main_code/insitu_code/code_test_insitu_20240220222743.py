...
indice={'temp':{'min':('Tnn','Tn10p'), 'max':('Txx','Tx90p'),'other':('DTR')},
        'precip':('CDD','CWD','R10mm','R20mm','R50mm','R1mm','Rx1day','Rx5day','SDII','Prcptot','Rptot','Rpday')}
...
def gen_base(var,start,end,dataset,percentile:list = None):
    ...
    var_no=param[var]
    fol=f'para_{var_no}/'
    ifile=f'{dataset}_{var_no}.nc' if var_no != 167 else f'{var}_{dataset}_{var_no}.'
    #base period
    ofile='b_'+ifile
...
class precip_ind:
    ...
    def __call__(self,ind,dataset):
        ...
        if ind in indice['temp'].values:
            self.fol='para_167/'
            if ind in indice['temp']['other']:
                self.iminfile=self.fol+f'min_{dataset}.nc'
                self.imaxfile=self.fol+f'max_{dataset}.nc'
            elif ind in indice['temp']['min']:
                self.ifile = self.fol+f'min_{dataset}.nc'
            else:
                self.ifile = self.fol+f'max_{dataset}.nc'
        if ind in indice['precip']:
            self.fol="para_228/"
