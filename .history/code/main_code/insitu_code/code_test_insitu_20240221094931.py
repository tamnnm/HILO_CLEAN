...
class gen_ind:
    ...
    def gen_gec_thres(self,thres=1):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gec,{thres} {self.ifile}',output=self.ofile)
    def gen_gtc_thres(self,thres=25):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gtc,{thres} {self.ifile}',output=self.ofile)
    def gen_lec_thres(self,thres=25):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gtc,{thres} {self.ifile}',output=self.ofile)
    def gen_ltc_thres(self,thres=25):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gtc,{thres} {self.ifile}',output=self.ofile)
    def gen_rx1day(self):
