...
def gen_base(var,start,end,dataset,percentile:list = None):
    ...
    if os.path.exist(ofile):
        if var=="tp":
            cdo.selyear('start+'/'+end',input=f'-setctomiss 0 -mul {ifile} -gec,1 {ifile}',output=fol+ofile,options='L')
        else:
...
def get_files(var_no,ind, dataset):
  fol = f'para_{var_no}/'

  if ind in indice[var]['other']:
    return fol + f'min_{dataset}_167.nc', fol + f'max_{dataset}_167.nc'

  elif ind in indice['167']['min']:
    return fol + f'min_{dataset}_167.nc', None

  else:
    return fol + f'max_{dataset}_167.nc', None


def get_228_file(dataset):
  fol = 'para_228/'
  return fol + f'{dataset}_228.nc
