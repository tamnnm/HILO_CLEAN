...
def get_name_files(dataset,var=None,ind=None, base: bool=False, rp: Optional[str] = None, window: Optional[int] = 5):
    if ind is not None:
    ...
    fol = f'para_{var_no}/'
    def get_name(fol,dataset,var_no,type_ind=None, base: bool=False, rp: Optional[str] = None):
        ...
        return fol+ifile,fol+ibfile,fol+ibminfile,fol+ibmaxfile
    iminfile,imaxfile=get_name(fol,dataset,type_ind='min')[0],get_name(fol,dataset,type_ind='max')[0]
    if base and rp is not None:
        ibminfile,ibmaxfile=get_name(fol=fol,dataset=dataset,type_ind='min',base=base,rp=rp)[0],get_name(fol=fol,dataset=dataset,type_ind='max',base=base,rp=rp)[0]
        
    if ind in indice[var_no]['other']:

        return fol, get_name(fol,dataset,type_ind='min')[0],get_name(fol,dataset,type_ind='max')[0]
    fol + f'min_{dataset}_{var_no}.nc', fol + f'max_{dataset}_{var_no}.nc'
