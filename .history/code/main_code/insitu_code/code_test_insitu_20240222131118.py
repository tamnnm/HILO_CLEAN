...
param = {'uwnd': {'var_no':'165', 'type':'non_extreme'},
         'vwnd': {'var_no':'166', 'type':'non_extreme'},
         'min': {'var_no':'167', 'type':'extreme'},
         'max': {'var_no':'167', 'type':'extreme'},
         't2m':{'var_no':'167', 'type':'non_extreme'}, #Not exist yet but will be
         'tp': {'var_no':'228', 'type':'extreme'},
         }
#endregion
...
def gen_base(dataset:Optional[Literal["cera","era","era5","noaa","obs"]],var:Optional[Literal['uwnd','vwnd','tp','min','max','t2m']],start:Optional[int],end:Optional[int],percentile:list = None,window='5'):
    #create base file
    var_no=param[var]
    fol=f'para_{var_no}/'
    ...
    if os.path.exist(ofile):
        ...
        if var_no=='228' and dataset != "obs":
            ...
            cdo.seldate(time_range,input=ifile,output=ofile,options='L')
            ...
            if percentile:
                ...
                for rp in percentile:
                    ...
                    if not check_exist(pfile):
                        ...
                        cdo.selyear(f'{start}/{end}', input=f'-ydrunpctl {rp},{window},pm=r8 {ibfile} {pminfile} {pmaxfile}',output=pfile)
    return


def get_name_files(dataset,var=None,ind=None, rp: Optional[str] = None):
    base= True if rp is not None else False
    if ind is not None:
        var_no=next(var_no for var_no in list(indice.keys()) if ind in indice[var_no])
    elif ind is not None:
        var_no=param[var]
    else:
        raise KeyError("Must type in index or variable")
    fol = f'para_{var_no}/'
    def get_name(fol=fol,dataset=dataset,var_no=var_no,type_ind=None, base=base, rp=rp):
        ifile= f'{dataset}_{var_no}.nc' if not type_ind else f'{type_ind}_{dataset}_{var_no}.nc'
        ofile= 'b_'+ifile
        #Use only when generating new base file a.k.a indice = none
        ibfile,ibminfile,ibmaxfile=f'b_{rp}_{ifile}',f'b{rp}p_min_{ifile}',f'b{rp}p_max_{ifile}' if base is True else None,None,None
        return ifile,ofile,ibfile,ibminfile,ibmaxfile
    if var_no=='167':
        # Only for extreme indice
        if ind is not None:
            iminfile,imaxfile=get_name(type_ind="min")[0],get_name(type_ind="max")[0]
            if ind in indice[var_no]['min']:
                imaxfile=None
            if ind in indice[var_no]['max']:
                iminfile=None
            return fol,iminfile,imaxfile
        # For normal extraction for ifile,basefile,ptclfile
        else:
            ifile,ofile,ibfile,ibminfile,ibmaxfile=get_name()
            return
    else:
        ifile,ofile=get_name()[:2]
