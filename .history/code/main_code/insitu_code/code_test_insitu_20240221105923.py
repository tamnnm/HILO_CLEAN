...
def gen_base(var,start:Optional[int],end:Optional[int],dataset,percentile:list = None,window=5):
    #create base file
    var_no=param[var]
    fol=f'para_{var_no}/'
        ...
        if var in param['extreme'].keys():
            if percentile:
                ...
                for rp in percentile:
                    ...
                    pminfile=fol+f'b{rp}p_min_{ofile}'
                    pmaxfile=fol+f'b{rp}p_max_{ofile}'
                    if not check_exist(pfile):
                        if not check_exist(pminfile): cdo.ydrunmin('5',input=ifile,output= pminfile)
                        if not check_exist(pmaxfile): cdo.ydrunmax('5',input=ifile,output= pmaxfile)
                        cdo.ydrunpctl(str(rp),'5',  input=f'{ibfile} -timin {ibfile} -timax{ibfile}',output=pfile)
    return
...
class gen_ind:
    ...
    def gen_compare_rptot(self):
        ...
        if 'T' in self.ind: percentile=self.ind[2:4]
        else: pass
        if int(percentile) < 50:
            if not check_exist(self.ofile): cdo.yearsum(input=f'-mul {self.ifile} -lc,0 -sub {self.bfile} {self.ifile}',output=self.ofile)
        else:
