...
def gen_base(ofile,start,end,dataset,var,percentile:list = None):
    #create base file for wet days
    org_name=f'para_{var}/{dataset}_{var}'
    ifile=f'{org_name}.nc'
    ofile,minfile,maxfile=f'n_{ifile}',f'x_{ifile}'
    if os.path.exist(ofile):
        if var=="tp":
            cdo.setrtomiss('0','0.999',input="-selyear,"+start+'/'+end+' '+ifile,output=ofile,options='L')
        else:
            cdo.selyear('start+'/'+end',input=ifile,output=ofile,options='L')
        if var in param['extreme'].keys():
            if check_exist(minfile): cdo.timmin(input=ifile,output=minfile)
            if check_exist(maxfile): cdo.timmax(input=ifile,output=maxfile)
            if percentile:
                for rp in percentile:
                    pfile=f'p{rp}_{ofile}'
                    if check_exist(pfile): cdo.timpctl(str(rp),input=ofile,output=pfile)

def gen_wet_days(ofile,start,end,dataset,var,threshold):
