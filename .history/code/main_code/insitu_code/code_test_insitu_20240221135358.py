...
class gen_ind:
    ...
    def gen_compare_rptot(self):
        ...
        if not check_exist(self.ofile): cdo.yearsum(input=f'-mul {self.ifile} -{method},0 -sub {self.ifile} {self.bfile}',output=self.ofile)
    def gen_compare_pctlday(self):
        if 'R' in self.ind: percentile=self.ind[1:3]
        elif 'T' in self.ind: percentile=self.ind[2:4]
        else: pass
        method="ltc" if int(percentile) < 50 else "gtc"
        if not check_exist(self.ofile): cdo.yearsum(input=f'-{method},0 -sub {self.ifile} {self.bfile}',output=self.ofile)
    def gen_min(self):
    ...
    def gen_temp_si(self):
        method="ltc" if self.ind=="wsdi" else "gtc"
        if not check_exist(self.ofile): cdo.yearsum(input=f'-{method},0 -sub {self.ifile} {self.bfile}',output=self.ofile)
