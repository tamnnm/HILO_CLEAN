...
def gen_base(var:Optional[list(param.keys())],start:Optional[int],end:Optional[int],dataset:Optional[Literal["cera","era","era5","noaa","obs"]],percentile:list = None,window='5'):
    #create base file
    var_no=param[var]
    fol=f'para_{var_no}/'
...
def get_ifiles(ind, dataset: Optional[Literal["cera","era","era5","noaa", "obs"]], base:bool, percentile: Optional[str] = None, window: Optional[int] = 5):
    var_no=next(var_no for var_no in list(indice.keys()) if ind in indice[var_no])
    fol = f'para_{var_no}/'
    def get_name(fol,dataset,var_no,type_ind=None, base:bool, window):
