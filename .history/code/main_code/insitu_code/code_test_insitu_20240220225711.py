...
def get_ifiles(var_no,ind, dataset):
  fol = f'para_{var_no}/'
  if ind in indice[var_no]['other']:
    return fol + f'min_{dataset}_{var_no}.nc', fol + f'max_{dataset}_{var_no}.nc'
  elif ind in indice[var_no]['min']:
    return fol + f'min_{dataset}_{var_no}.nc', None
  elif ind in indice[var_no]['max']:
    return fol + f'max_{dataset}_{var_no}.nc', None
  else:
    return fol + f'{dataset}_{var_no}.nc', None
...
class gen_ind:
    ...
    def __call__(self,ind,dataset):
        ...
        if ind in indice['167'].values:
            self.fol='para_167/'
            if ind in indice['167']['other']:
                self.iminfile=self.fol+f'min_{dataset}_167.nc'
                self.imaxfile=self.fol+f'max_{dataset}_167.nc'
            elif ind in indice['167']['min']:
                self.ifile = self.fol+f'min_{dataset}_167.nc'
            else:
                self.ifile = self.fol+f'max_{dataset}_167.nc'
        if ind in indice['228']:
            self.fol="para_228/"
            self.ifile = self.fol+f'{dataset}_228.nc'
        self.ofile = self.fol+'{ind}_{dataset}.nc'
    ...
    def gen_prcptot(self):
        if not check_exist(self.ofile):
            cdo.yearsum(input=f'-mul {self.ifile} -gec,1 {self.ifile}',output=self.ofile)
    def gen_rptot(self):
        if not check_exist(self.ofile):
            cdo.yearsum(input=f'-mul {self.ifile} -gtc,0 -sub {self.ifile} {self.bfile}',output=self.ofile)
    def gen_rpday(self):
        if not check_exist(self.ofile):
            cdo.yearsum(input=f'-gtc,0 -sub {self.ifile} {self.bfile}',output=self.ofile)
