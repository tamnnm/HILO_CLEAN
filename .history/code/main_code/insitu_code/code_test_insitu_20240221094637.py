...
def gen_base(var,start,end,dataset,percentile:list = None):
    ...
    if os.path.exist(ofile):
        if var=="tp":
            cdo.selyear(f'{start}/{end}',input=f'-setctomiss 0 -mul {ifile} -gec,1 {ifile}',output=fol+ofile,options='L')
        else:
            cdo.selyear(f'{start}/{end}',input=ifile,output=ofile,options='L')
        if var in param['extreme'].keys():
            if percentile:
                ibfile=fol+ofile
                # percentile of base value
                for rp in percentile:
                    pfile=fol+f'b{rp}p_{ofile}'
...
class gen_ind:
    ...
    def gen_cdd(self):
        if not check_exist(self.ofile): cdo.yearmax(input=f'-consects -lc,1 {self.ifile}',output=self.ofile)
    def gen_cwd(self):
        if not check_exist(self.ofile): cdo.yearmax(input=f'-consects -gec,1 {self.ifile}',output=self.ofile)
    def gen_r10mm(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gec,10 {self.ifile}',output=self.ofile)
    def gen_r20mm(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gec,20 {self.ifile}',output=self.ofile)
    def gen_r50mm(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gec,50 {self.ifile}',output=self.ofile)
    def gen_r1mm(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gec,1 {self.ifile}',output=self.ofile)
    def gen_rx1day(self):
        if not check_exist(self.ofile): cdo.monmax(input=f'{self.ifile}',output=self.ofile)
    def gen_rx5day(self):
        if not check_exist(self.ofile): cdo.monmax(input=f'-runsum,5 {self.ifile}',output=self.ofile)
    def gen_sdii(self):
        if not check_exist(self.ofile): cdo.yearmean(input=f'{self.ifile}',output=self.ofile)
    def gen_prcptot(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-mul {self.ifile} -gec,1 {self.ifile}',output=self.ofile)
    def gen_lt_rptot(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-mul {self.ifile} -gtc,0 -sub {self.bfile} {self.ifile}',output=self.ofile)
    def gen_gt_rptot(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-mul {self.ifile} -gtc,0 -sub {self.ifile} {self.bfile}',output=self.ofile)
    def gen_lt_pctlday(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gtc,0 -sub {self.bfile} {self.ifile}',output=self.ofile)
    def gen_gt_pctlday(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gtc,0 -sub {self.ifile} {self.bfile}',output=self.ofile)
    def gen_lt_per_pctl(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gtc,0 -sub {self.bfile} {self.ifile}',output=self.ofile)
    def gen_gt_per_pctl(self):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-gtc,0 -sub {self.ifile} {self.bfile}',output=self.ofile)
    def gen_min(self):
    ...
    def gen_lt_thres(self,thres='25'):
        if not check_exist(self.ofile): cdo.yearsom(input=f'-sub {self.imaxfile} {self.iminfile}',output=self.ofile)
    def gen_gt_thres(self,thres='20'):
