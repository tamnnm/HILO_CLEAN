...
def gen_base(var,start:Optional[int],end:Optional[int],dataset,percentile:list = None,window='5'):
    ...
    if os.path.exist(ofile):
        ...
        if var=="tp":
            cdo.seldate(time_range,input=f'-setct{omiss 0 -mul {ifile} -gec,1 {ifile}',output=fol+ofile,options='L')
        else:
            ...
            if percentile:
                ...
                for rp in percentile:
                    ...
                    if not check_exist(pfile):
                        ...
                        cdo.seldate('' input=f'-ydrunpctl {rp},{window},pm=r8 {ibfile} {pminfile} {pmaxfile}',output=pfile)
    return
