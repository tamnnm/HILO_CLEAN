...
class gen_ind:
    ...
    def gen_temp_si(self):
        ...
        if not check_exist(self.ofile): cdo(input=cdo.yearmax(input=f'-consects '-{method},0 -sub {self.ifile} {self.ibfile}',output=self.ofile)

        if not check_exist(self.ofile): cdo(input=fcdo.yearmax(input=f'-consects '-{method},0 -sub {self.ifile} {self.ibfile}',output=self.ofile)
