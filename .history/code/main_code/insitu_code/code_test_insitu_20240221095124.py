...
class gen_ind:
    ...
    def gen_compare_rptot(self,method):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-mul {self.ifile} -{method},0 -sub {self.bfile} {self.ifile}',output=self.ofile)
    def gen_compare_pctlday(self,method):
        if not check_exist(self.ofile): cdo.yearsum(input=f'-{method},0 -sub {self.bfile} {self.ifile}',output=self.ofile)
    def gen_min(self):
