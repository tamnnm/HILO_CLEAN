...
class precip_ind:
    ...
    def __call__(self,ind,dataset):
        ...
        if ind == "cdd":
            self.gen_cdd()
    def gen_cdd(self):
        ofile=f'{self.fol}/cdd_{self.dataset}.nc'
        if not check_exist(ofile):
            cdo.yearmax(input=f'-consects -lc,1 {self.ifile}',output=ofile)
    def gen_cwd(self):
        ofile=f'{self.fol}/cwd_{self.dataset}.nc'
        if not check_exist(ofile):
            cdo.yearmax(input=f'-consects -gtc,1 {self.ifile}',output=ofile)
    def gen_r10mm(self):
        ofile=f'{self.fol}/r10mm_{self.dataset}.nc'
        if not check_exist(ofile):
            cdo.yearsum(input=f'-gtc,10 {self.ifile}',output=ofile)
    def gen_r20mm(self):
        ofile=f'{self.fol}/r20mm_{self.dataset}.nc'
        if not check_exist(ofile):
            cdo.yearsum(input=f'-gtc,20 {self.ifile}',output=ofile)
    def gen_r50mm(self):
