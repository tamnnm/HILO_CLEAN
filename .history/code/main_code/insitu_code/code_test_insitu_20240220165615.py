...
def check_exist(name):
    return True if os.path.exists(name) else
...
def gen_base(ofile,start,end,dataset,var):
    ...
    if os.path.exist(ofile):
        ...
        if var in param['extreme'].keys():
           if var=="tp":
               minfile,maxfile =f'n_{org_name}.nc',f'x_{org_name}.nc'
                if
                cdo.timmin(input=ifile,output=f'n_{org_name}.nc')
                cdo.timmax(input=ifile,output=f'x_{org_name}.nc')
                cdo.timpctl('99',input=ofile,)
           cdo.timpctl()
