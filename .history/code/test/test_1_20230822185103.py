...
#Method 2
date_generated=pd.Series(pd.date_range(start=date_start,end=date_end),freq='2H')
...
if len(gtest_vl) == len(rdata_flat):
    sum_data=pd.DataFrame(dict(rdata=rdata_flat, gdata=gtest_vl),index=date_generated)
else:
    print("The dimension are not qua")
#"""
...
plt.plot(date_generated, rdata_flat)
