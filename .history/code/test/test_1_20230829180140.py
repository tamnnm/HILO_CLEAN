...
# Method 2
date_generated = pd.Series(pd.date_range(
    start=date_start, end=date_end), freq='2H')
...
for idx in closest_pts[:, 1]:
    ...
    gtest_vl = ds.sel(longitude=lon_pts, latitude=lat_pts,
                      time=(date_start, date_end))['t2m'].values
...
#temperature
txx=tnx=txn=tnn=sud=tr20=tn10p=tx10p=tn90p=tx90p=wsdi=csdi=gsl=dtr=0
...
if len(gtest_vl) == len(rdata_flat):
    sum_data = pd.DataFrame(
        dict(rdata=rdata_flat, gdata=gtest_vl), index=date_generated)
    ...
    sum_data.reset_index().plot(x='date_generated', y=['rdata', 'gdata'])
