...
with open(Data_wd+"euler_pts.txt",'r') as file:
    station = f.read()
    lines = pd.DataFrame(station.split("\n"))
    f.close()

# --------- combine the grid and insitu data into dataframe --------- #
gtest_all =[] #all gridded data
gtest_avg=[] #average of gridded data
for i,line in enumerate(lines):
    if city_name in line:
        ...
        for i in np.range(0,len(euler_pts),2):
            ...
            gtest_all.append(gtest_all)
        gtest_avg=np.mean(gtest_all,axis=0)
        #axis 0: verticle
        #axis 1: horizontal
        if len(gtest_avg) == len(rdata_flat):
            all_data = pd.DataFrame(rdata_flat+gtest_avg).replace("-99.99", np.NaN)
            ...
            all_data['mean']=all_data.iloc[:,1:].mean(axis=1)
            print(len(all_data))
    ...
    print("Please check again whether or not the country is valid")
