...
for i, line in enumerate(lines[2:]):
    ...
    if i % 32 != 0:
        data_clean.append(temp_line[1:])
        del temp_line
    else:
        del temp_line
        continue

data_flat = list(itertools.chain(data_clean))
print(data_flat)

# ------------- format the date -> only show the years ------------- #

year_start = int(data_row[0][0])
...
plt.plot(date_generated, data_flat)
