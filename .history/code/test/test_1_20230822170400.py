...
start = dt.datetime.strptime(f'01-01-{year_start}', "%d-%m-%Y")
end = dt.datetime.strptime(f'31-12-{year_end}', "%d-%m-%Y")
date_generated = [
    start + dt.datetime.timedelta(days=x) for x in range(0, (end-start).days)]

#U can also use pd.range then put the data into a dataframe
