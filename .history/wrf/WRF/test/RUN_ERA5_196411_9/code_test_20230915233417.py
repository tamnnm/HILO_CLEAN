...
p_test=[200,500,850]
for z_p in p_test:
    time_co=cut_time(z_p,1964,11)
    data_test=cut_mlp(z_p,time_co)
    range=5
    break_factor=0
    while range<=5 and break_factor<=0:
        try:
            dset_plot=data_test.where(abs(data_test.values - z)<=range)
            break_factor=1
        except:
            range=range+1
    ...
    for i in range(0,31):
        ...
        ax.set_title(f'{i}/11/1964',pad=25)
        plt.savefig(os.path.abspath(rf'{Data_path}/{i}.jpg'),format="jpg",dpi=300)
