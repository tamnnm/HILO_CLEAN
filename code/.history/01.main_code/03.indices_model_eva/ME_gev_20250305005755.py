...
for j,ax in enumerate(axes.flatten()):
    for i in range(12):
        ...
        if i_ri%3 == 0: # or i_ri == 1:#(i_ri == 1 and j%2 != 0):
            ...
            #     sc = ax.scatter(valid_data['shape'], valid_data[indice_list[int(j//2)]], c=valid_data['scale'], marker=markers[order], cmap='Spectral', s=200, edgecolors='k')
            # else:
            #     max_value = max_20 if '20' in indice_list[int(j//2)] else max_50
            #     #!! I give up. I have to input the max_value to get this to work
            #     sc = ax.scatter(valid_data['shape'], max_value - valid_data[indice_list[int(j//2)]], c=valid_data['scale'], marker=markers[order], cmap='Spectral', s=200, edgecolors='k')
...
mu=40 #????????????????
k = np.linspace(-1,1.02,1000)
DTi_20 = np.linspace(0.01,50.1,1000)
DTi_50 = np.linspace(0.01,50.1, 1000)
[K_20,DTi_20] = np.meshgrid(k,DTi_20)
[K_50,DTi_50] = np.meshgrid(k,DTi_50)
# SIG_20 = np.zeros(K_20.shape)
# SIG_50 = np.zeros(K_50.shape)

SIG_20 = np.empty(K_20.shape)
SIG_20[:] = np.nan

SIG_50 = np.empty(K_50.shape)
SIG_50[:] = np.nan
...
for j, ax in enumerate(axes.flatten()):
    if indice_list[int(j//2)] == 'DT_20' and j%2 == 0:
        CS = ax.contourf(K_20,DTi_20,SIG_20,level = tmp_20)
        ax.clabel(CS, inline=1, fontsize=50)
    if indice_list[int(j//2)] == 'DT_50' and j%2 == 0:
        CS = ax.contourf(K_50,DTi_50,SIG_50, level = tmp_50)
        ax.clabel(CS, inline=1, fontsize=50)
...
# cbar.set_label(r'$\sigma (^\circ C)$', rotation=0, fontsize=30, labelpad=10)
