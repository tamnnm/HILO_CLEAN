...
def plot_dom():
    fig, axes = plt.subplots(2,2, figsize = (20,45))
    for i,ax in enumerate(axes.flatten()):
        # Plot donut chart
        all = dom_seas_all[:,0][i::4]
        base = dom_seas_base[:,0][i::4]
        now = dom_seas_now[:,0][i::4]
        ax.pie([all.count(0), all.count.count(1)], labels = ['Frequency', 'Intensity'], autopct='%1.1f%%', colors = ['#bd004e', '#142354'], startangle=90)
        ax.pie([base.count(0), base.count.count(1)], labels = ['Frequency', 'Intensity'],
               autopct='%1.1f%%', colors = ['#bd004e', '#142354'], startangle=90)
        ax.pie([now.count(0), now.count.count(1)], labels = ['Frequency', 'Intensity'], autopct='%1.1f%%', colors = ['#bd004e', '#142354'], startangle=90)
