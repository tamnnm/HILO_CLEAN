...
def cluster_plot(resample_mode = 3):
    ...
    # Rectangle frame of the map
    def rec_bound(limit):
        return Polygon([(limit[0], limit[2]), (limit[0], limit[3]), (limit[1], limit[3]), (limit[1], limit[2])])
    
    # Plot the topography
    mask = mask_create(vnmap, geo_limit)
    stamen_terrain = cimgt.Stamen('terrain-background')
    st_proj = stamen_terrain.crs
    mask
    
    ax_1.add_image(stamen_terrain, 7, zorder=0) # 7 is in meters
