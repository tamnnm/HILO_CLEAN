...
def cluster_plot(resample_mode = 3):
    ...
    # ----------------------- Plot the shapefile ----------------------- #
    # Only the border
    for polygon in vnmap:
        x,y = polygon.exterior.xy
        ax_1.plot(x, y, color='black', linewidth=1, alpha=0.5)
        
    # Create custom mask
    # Custom lon_array & lat_array based on your real data (Apply for contour plot)
    # If no lon, lat is specified, it will create a default one based on limit - extend of the plot
    # polygons extracted from shapefile
    #? If you want to custom the exact city, please change polygons
    # Default value is whole Vietnam & islands
    def mask_create(polygons,limit = [None,None,None,None], lon_array = [None], lat_array = [None]):
        """
        Create a custom mask for contour plots based on specified polygons and coordinate arrays.

        Parameters:
        polygons (list): List of shapely.geometry.Polygon objects representing the areas to be masked.
        limit (list): List of four elements specifying the extent of the plot [lon_min, lon_max, lat_min, lat_max].
                Default is [None, None, None, None]. If no lon_array and lat_array are specified, this limit
                will be used to create default coordinate arrays.
        lon_array (list or numpy.ndarray): Array of longitudes. Default is [None]. If not specified, it will be
                                        created based on the limit.
        lat_array (list or numpy.ndarray): Array of latitudes. Default is [None]. If not specified, it will be
                                        created based on the limit.

        Returns:
        numpy.ndarray: A boolean mask array where True indicates the points outside the specified polygons.

        Raises:
        ValueError: If both lon_array and lat_array are not specified and limit is not provided.
        """
        
        # No lon, lat is specified
        if lon_array[0] == None and lat_array[0] == None:
            if limit == [None,None,None,None]:
                raise ValueError("Please specify the limit or lon, lat array")
            else:
                lon_array = np.linspace(limit[0], limit[1], 100)
                lat_array = np.linspace(limit[2], limit[3], 100)
        mask_array = np.zeros((len(lat_array), len(lon_array)), dtype=bool)
        
        for polygon in polygons:
            for i, lon in enumerate(lon_array):
                for j, lat in enumerate(lat_array):
                    mask_array[i, j] = not polygon.contains(Point(lon, lat))
        return mask
