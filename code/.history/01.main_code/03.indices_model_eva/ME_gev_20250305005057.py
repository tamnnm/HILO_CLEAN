...
for i,j in zip(range(K_20.shape[0]),range(K_20.shape[1])):
    SIG_20[i,j] = root_scalar(lambda sig: DT(20,mu,sig*(sig>0)+1e-5,K_20[i,j])-DTi_20[i,j], bracket=[1e-5,1000]).root
    SIG_50[i,j] = root_scalar(lambda sig: DT(50,mu,sig*(sig>0)+1e-5,K_50[i,j])-DTi_50[i,j], bracket=[1e-5,1000]).root
