...
DTi_20 = np.linspace(0.01,40, 100)
DTi_50 = np.linspace(0.01,40, 100)
...
with multiprocessing.Pool() as pool:
    for i, j in indices:
        futures.append(pool.apply_async(compute_SIG, args=(i, j)))
    pool.close()
    for f in futures:
            f.wait()
            print("Progress: {:.2f}%".format(len(futures) - len([f for f in futures if not f.ready()])) + " of {} tasks".format(len(futures)))
            time.sleep(1)
