...
def plot_dom():
    ...
    for i,ax in enumerate(axes.flatten()):
        ...
        angle = 90
        textprops = {'color': 'w','rotation' : 20 if i == 0 else 60, 'fontsize': 35}
        # print(all.shape)
        # raise KeyboardInterrupt
        ax.pie([all.count(0), all.count(1)],# labels = ['Frequency', 'Intensity'],
            autopct='%1.1f%%', colors = ['#bd004e', '#142354'], startangle=angle,
            radius=1.5,pctdistance = 0.9, textprops = textprops,
            wedgeprops=dict(width=0.7, edgecolor = 'w', linewidth = 10))
        ax.pie([now.count(0), now.count(1)],# labels = ['Frequency', 'Intensity'],
            autopct='%1.1f%%',pctdistance = 0.82, colors = ['#bd004e', '#142354'], startangle=angle,
            radius=1.2,textprops = textprops,
            wedgeprops=dict(width=0.6,edgecolor = 'w', linewidth = 10))
        ax.pie([base.count(0), base.count(1)],# labels = ['Frequency', 'Intensity'],
               colors = ['#bd004e', '#142354'], startangle=angle,
               radius=0.9,autopct='%1.1f%%', pctdistance = 0.7,textprops = textprops,
