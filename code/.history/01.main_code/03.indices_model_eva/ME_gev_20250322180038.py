...
from my_junk import mask_create
# import re
...
def cluster_plot(resample_mode = 3):
    ...
    # -------------------- DEM plot ---------------------------- #
    def plot_dem(ax, mask=True, polygons = polygons):
            ...
            if polygons is None:
                ...
                for geom in vnmap.geometry:
                    if isinstance(geom, Polygon):
                        ...
                        continue
            mask = mask_create(polygons, lon_array = rastercoords[..., 0][0], lat_array = rastercoords[..., 1][0])
            rastercoords_fin = np.ma.masked_array(rastervalues, mask)
        else:
            rastercoords_fin = rastercoords
            
            
        # specify kwargs for plotting, using terrain colormap and LogNorm
        dem = ax.pcolormesh(
            np.meshgrid(rastercoords_fin[..., 0], rastercoords_fin[..., 1]),
            rastercoords_fin
            cmap=plt.cm.terrain,
