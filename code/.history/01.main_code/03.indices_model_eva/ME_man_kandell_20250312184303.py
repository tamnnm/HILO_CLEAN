...
def point_map(par:Optional[Literal['R','T2m']] = 'R',dataset:Optional[Literal['rean', 'obs']]= 'rean', direction = 'vertical'):
    ...
    if direction == "vertical":
        ...
        gs_main = gridspec.GridSpecFromSubplotSpec(no_row, no_col,\
                                                    ...
                                                    subplot_spec=gs[0],\
                                                    wspace= -0.8 if par == 'T2m' else -0.7, hspace=-0.05)
    ...
    last_row_no = no_row * no_col - len(var_tuple)
    ...
    for ax in axs:
        ...
        if ax.get_subplotspec().get_geometry()[1] != 1 \
            and ((ax.get_subplotspec().rowspan.start == no_row - 1 and ax.get_subplotspec().colspan.start < (no_row * no_col - 1 - len(var_tuple) if par == 'R' else no_row * no_col - len(var_tuple)))
                or
