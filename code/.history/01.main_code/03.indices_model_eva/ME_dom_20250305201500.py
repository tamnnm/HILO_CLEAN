...
def plot_dom():
    ...
    for i,ax in enumerate(axes.flatten()):
        ...
        all = dom_seas_all[:,i,0]#[i::4].tolist()
        base = dom_seas_base[:,i,0]#[i::4].tolist()
        ## Now starts from DJF not MAM
        now = dom_seas_now[:,i+1 if i+1 < len(axes.flatten()) else 0,0]#[(i+1 if i+1<len(axes.flatten()) else 0)::4].tolist()
        angle = 60
