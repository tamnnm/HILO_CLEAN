...
def point_map(par:Optional[Literal['R','T']] = 'R',dataset:Optional[Literal['rean', 'obs']]= 'rean', direction = 'verticle'):
    ...
    for ax in axs:
        ...
        if dataset == 'obs':
            for station in name_list:
                ...
                scatter=ax.scatter(position[0], position[1], s=150, facecolor=colors, marker=markers, edgecolors=edge_colors, alpha = 0.75, linewidth=2, transform=ccrs.PlateCarree())
    ...
    legend = ax.legend()
    fig.tight_layout()
    fig.savefig(f'/work/users/tamnnm/code/02.sup/image/{par}_MK_{dataset}_{direction}.jpg',
...
def violin(par):
    ...
    fig.savefig(f'/work/users/tamnnm/code/02.sup/image/violin_{par}.jpg',
                format="jpg", bbox_inches='tight', dpi=200)
    return
