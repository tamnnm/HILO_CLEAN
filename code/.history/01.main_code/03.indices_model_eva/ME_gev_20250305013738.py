...
DTi_20 = np.linspace(-1,50, 1000)
DTi_50 = np.linspace(-1,50, 1000)
...
for j, ax in enumerate(axes.flatten()):
    if indice_list[int(j/2)] == 'DT_20':
        CS = ax.contour(K_20,DTi_20,SIG_20,level = 1, colors='black',linestyles='dashed')
        ax.clabel(CS, inline=1, fontsize=10)
    if indice_list[int(j/2)] == 'DT_50':
        CS = ax.contour(K_50,DTi_50,SIG_50,level = 1,colors='black',linestyles='dashed')
        ax.clabel(CS, inline=1, fontsize=10)
    # if j%2 != 0:
    #     ax.set_xscale('log')
