    ...
    for i in range(12):
        ...
        if i_ri%3 == 0: # or i_ri == 1:#(i_ri == 1 and j%2 != 0):
            ...
            if 'MST' in indice_list[int(j//2)]:
                sc = ax.scatter(valid_data['shape'], valid_data[indice_list[int(j//2)]], c=valid_data['scale'], marker=markers[order], cmap='Spectral', s=200, edgecolors=edgecolors[order])
            else:
                max_value = max_20 if '20' in indice_list[int(j//2)] else max_50
                #!! I give up. I have to input the max_value to get this to work
                sc = ax.scatter(valid_data['shape'], max_value - valid_data[indice_list[int(j//2)]], c=valid_data['scale'], marker=markers[order], cmap='Spectral', s=200, edgecolors=edgecolors[order])
