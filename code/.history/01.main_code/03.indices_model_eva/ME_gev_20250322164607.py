...
def cluster_plot(resample_mode = 3):
    ...
    combined_geom = vnmap.unary_union
    
    # Only the border
    for geom in vnmap.geometry:
        if isinstance(geom, Polygon):
            # List of all points of a single polygon
            polygons = [geom]
        elif isinstance(geom, MultiPolygon):
            # List of all points of multiple polygons
            polygons = list(geom)
        else:
            continue
    ...
    # Create a mask by subtracting the combined geometry from the boundary polygon
    mask = rec_bound(geo_limit).difference(combined_geom)
    
    # Project the mask to the stamen terrain projection
    mask_stamen = st_proj.project_geometry(mask, src_crs=map_pro, dst_crs=st_proj)
    
    ax_1.add_image(stamen_terrain, 7, zorder=0) # 7 is in meters
    ax_1.add_geometries([mask_stamen], crs=st_proj, facecolor='transparent', edgecolor='none', alpha=0, zorder=1)
