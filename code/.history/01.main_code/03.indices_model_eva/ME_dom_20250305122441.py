...
def process_station(no_station, pcp_subset, no_wet_subset, sdii_subset):
    pcp_ind = pcp_subset.sel(no_station = no_station).values[1::]
    no_wet_ind = no_wet_subset.sel(no_station = no_station).values[1::]
    sdii_ind = sdii_subset.sel(no_station = no_station).values[1::]
    
    dom_seas = []
    
    for i in range(4):
        no_wet_ind = no_wet_ind[i::4]
        sdii_ind = sdii_ind[i::4]
        pcp_ind = pcp_ind[i::4]
        nonnan_index = np.where(~np.isnan(no_wet_ind) & ~np.isnan(sdii_ind) & ~np.isnan(pcp_ind))
        no_wet_ind_clean= no_wet_ind[nonnan_index]
        sdii_ind_clean = sdii_ind[nonnan_index]
        ...
        params = curve_fit(pcp_func, (no_wet_ind_clean, sdii_ind_clean), ydata, maxfev = 10000)
        print(params)
        s_n = abs(params[0])*no_wet_std
        s_i = abs(params[1])*sdii_std
        dom_seas.append([0 if s_n > s_i else 1, s_n, s_i])
    return dom_seas
...
print(dom_seas_all, dom_seas_all)
