...
for i in range(12):
    ...
    # else:
        ...
        final_data = pd.concat([data['name_station'], data['scale'], data['shape'], data['DT_20'], data['DT_50'], data['DT_MST']], axis=1)
        # print(ri, data['DT_20'],data['DT_50'],data['DT_MST'])
    # raise KeyboardInterrupt
        final_datadata.to_csv(csv_name)
    if i > 1:
    ...
    for j,ax in enumerate(axes.flatten()):
        if i_ri%3 == 0 or (i_ri == 1 and j%2 != 0):
            order = int(i/3)
            print(i_ri,j,order)
            print(data)
