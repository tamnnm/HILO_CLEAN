...
legend_elements = [mlines.Line2D([0], [0], marker=markers[i], color='w', markeredgecolor=edgecolors[i], label=f'ri={1}' if i==0 else f'ri={i*3}', fontsize = 25, markerfacecolor=None, markersize=10) for i in range(len(markers))]
...
cbar = fig.colorbar(sc, ax=axes, orientation='horizontal', pad=0.15)
cbar.ax.set_aspect(0.01, adjustable='box')  # Adjust the aspect ratio to change the height
cbar.set_label(r'$\sigma (^\circ C)$', rotation=0, fontsize=40, labelpad=5)
