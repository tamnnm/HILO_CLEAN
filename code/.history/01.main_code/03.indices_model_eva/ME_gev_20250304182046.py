...
def x_inv(E, loc, scale, shape):
    ...
    """
    return gev.ppf(1-E, c=shape, loc=loc, scale=scale)
...
def x_Tr(yr, loc, scale, shape, ri = ri()):
    ...
    return gev.ppf(1-(ri/yr), c=shape, loc=loc, scale=scale)
...
def fit_som(gev_df, og = True, ind = "Tx"):
    ...
    columns = ['shape','loc','scale','lon','lat']
...
def fit_gev(ind,og=False,resample_mode = resample_mode):
    ...
    def sort_top(data, axis=None, resample_mode = resample_mode, **kwargs):
        ...
        return sorted_data[:resample_mode]
    ...
    gev_df.to_csv(f"gev_{ind}/{og_suffix}_{resample_mode}.csv")
    return gev_df
    

# for i in range(12): fit_gev('Tx', og = True, resample_mode = i+1)
...
# input
futures = []
var_tuple = ['Tx'] #rain_tuple or temp_tuple or custom_list
# output
with con_fu.ProcessPoolExecutor() as executor:
    for ind in var_tuple:
        futures+=[executor.submit(fit_gev, ind, og=True, resample_mode=i+1) for i in range(12)]
    con_fu.wait(futures)
"""
#endregion
...
min_20, max_20, min_50, max_50, min_shap, max_shape = 10, 0, 10, 0, 10, 0
for i in range(12):
    i = i+1
    # Run fit check
    csv_name = f"gev_Tx/OG_DT_{i}.csv"
    if os.path.exists(csv_name):
        data = pd.read_csv(csv_name)
    else:
        # data = fit_gev("Tx", og=True, resample_mode=i)
        data = pd.read_csv(f"gev_Tx/OG_{i}.csv")
        ri = 1/(i)
        data['DT_20'] = DT(20, data['loc'], data['scale'], data['shape'], ri=ri)
        data['DT_50'] = DT(50, data['loc'], data['scale'], data['shape'], ri=ri)
        print(x_Tr(1, data['loc'], data['scale'], data['shape'], ri=ri))
        data['DT_MST'] = x_Tr(1, data['loc'], data['scale'], data['shape'], ri=ri) - data['MST']
        data_new = pd.concat([data['loc'], data['scale'], data['shape'], data['DT_20'], data['DT_50'], data['DT_MST']], axis=1)
    ...
    if i > 1:
        ...
        min_shape = data['shape'].min() if data['shape'].min() < min_shap else min_shap
        max_shape = data['shape'].max() if data['shape'].max() > max_shape else max_shape
    
    
    order = int(i/3)
    for j,ax in enumerate(axes.flatten()):
        if i%3 == 0 or (i == 1 and j%2 != 0):
            valid_data = data.dropna(subset=[indice_list[int(j/2)]])
            # Remember to check MST as a single value only
            sc = ax.scatter(valid_data['shape'], valid_data[indice_list[int(j/2)]], c=valid_data['scale'], marker=markers[order], cmap='Spectral', s=100, edgecolors=edgecolors[order])

    # Set the title inside the graphing area
    ax.text(0.05, 1.1, indice_plot_list[int(j/2)], transform=ax.transAxes,
...
print(min_20,max_20,min_50,max_50)
...
for i,j in zip(range(K_20.shape[0]),range(K_20.shape[1])):
    SIG_20[i,j] = root_scalar(lambda sig: DT(20,mu,sig*(sig>0)+1e-6,K_20[i,j])-DTi_20[i,j], bracket=[1e-5,1000]).root
    SIG_50[i,j] = root_scalar(lambda sig: DT(50,mu,sig*(sig>0)+1e-6,K_50[i,j])-DTi_50[i,j], bracket=[1e-5,1000]).root

tmp_20=np.linspace(SIG_20.min(),SIG_20.max(),10)
tmp_50=np.linspace(SIG_50.min(),SIG_50.max(),10)

for j, ax in enumerate(axes.flatten()):
    if indice_list[int(j/2)] == 'DT_20' and j%2 == 0:
        CS = ax.contour(K_20,DTi_20,SIG_20,level = tmp_20, colors='black',linestyles='dashed')
        ax.clabel(CS, inline=1, fontsize=10)
    if indice_list[int(j/2)] == 'DT_50' and j%2 == 0:
        CS = ax.contour(K_50,DTi_50,SIG_50,level = tmp_50,colors='black',linestyles='dashed')
        ax.clabel(CS, inline=1, fontsize=10)
    ax.set_xscale('log')
    ax.xaxis.set_major_formatter(ticker.FuncFormatter(myLogFormat))
# Create custom legend for markers
legend_elements = [mlines.Line2D([0], [0], marker=markers[i], color='w', markeredgecolor=edgecolors[i], label=f'ri={1}' if i==0 else f'ri={i*3}', markerfacecolor=None, markersize=10) for i in range(len(markers))]
...
cbar = fig.colorbar(sc, ax=axes, orientation='horizontal', pad =0.075)
cbar.ax.set_aspect(0.025, adjustable='box')  # Adjust the aspect ratio to change the height
cbar.set_label(r'$\sigma (^\circ C)$', rotation=0, fontsize=25, labelpad=10)

fig.savefig(os.getenv('img')+"/Delta_GEV_test.png", bbox_inches='tight')
# axes = axes.flatten()
plt.close(fig)
...
def cluster_plot(resample_mode = 3):
    ...
    gs = fig.add_gridspec(1, 2, width_ratios=[3.7, 1], wspace=0.05)
    ...
    ax_1.set_xticks(np.linspace(101, 115, 5), crs=map_pro)
    ...
    ax_1.tick_params(axis='both', which='major',
                    labelsize=30, direction='in', length=20)
    ...
    for zone in txt_shapes:
        ...
        ax_1.plot(x, y, color="k", markersize=10e-6,
    ...
    ax_1.patch.set(lw=2, ec='k', alpha=0.5)
    ...
    # Set the title inside the graphing area
    ax_1.scatter(data['lon'], data['lat'], c=data['cluster'], cmap=cluster_cmap, s=300)

    ax2.scatter(data['shape'], data['loc'], c=data['cluster'], cmap=cluster_cmap, s=350, alpha = 0.5)
    ax2.set_xlabel(r'$\mathrm{k}$')
    ax2.set_ylabel(r'$\mu$')

    ax3.scatter(data['loc'], data['scale'], c=data['cluster'], cmap=cluster_cmap, s=350, alpha = 0.5)
    ax3.set_xlabel(r'$\mu$')
    ax3.set_ylabel(r'$\sigma$')
    ax4.scatter(data['scale'], data['shape'], c=data['cluster'], cmap=cluster_cmap, s=350, alpha=0.5)
    ax4.set_xlabel(r'$\sigma$')
    ax4.set_ylabel(r'$\mathrm{k}$')
    fig.tight_layout()
    fig.savefig(os.getenv('img')+f"/SOM_cluster_map_{resample_mode}.png", bbox_inches='tight')
#"""
#endregion
...
def doubling(resample_mode, gev_fit=False, doubling: Optional[Literal['linear', 'log', 'both']] = 'linear'):
    ...
    for i,name_station in enumerate(raw_data_dict.keys()):
        ...
        if not gev_fit:
            ...
            # Value of x that has return period: 50 years (x_50)
                x0 = x_Tr(TR, mu_org, sigma_org, k_org, ri(0)) # Reverse of cdf at TR
            ...
            # EXTREME DATA (n events/year)
                ...
                for j, mu in enumerate(mu_T):
                    ...
                    TR_rise[j,i] = ri(resample_mode = resample_mode)/E_rise[j,i]
    ...
    ax.annotate("",xy=(0.75, 0.28), xycoords = 'axes fraction', xytext = (0.45, 0.4), textcoords = 'axes fraction', arrowprops = {'arrowstyle':'simple', 'facecolor': 'black','linewidth': 6, 'connectionstyle':"arc3,rad=-0.2"})
    # # ax configuration
    ...
    fig.savefig(os.getenv('img')+f"/Doubling_rate_{resample_mode}.png")
