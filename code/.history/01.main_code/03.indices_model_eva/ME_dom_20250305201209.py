...
def process_station(option,no_station, pcp_subset, no_wet_subset, sdii_subset):
    if option != "now":
        pcp_subset = pcp_subset.sel(no_station = no_station).values[1::]
        no_wet_subset = no_wet_subset.sel(no_station = no_station).values[1::]
        sdii_subset = sdii_subset.sel(no_station = no_station).values[1::]
    else:
        pcp_subset = pcp_subset.sel(no_station = no_station).values
        no_wet_subset = no_wet_subset.sel(no_station = no_station).values
        sdii_subset = sdii_subset.sel(no_station = no_station).values
...
def plot_dom():
    ...
    for i,ax in enumerate(axes.flatten()):
        ...
        #!!! CHECK THIS. ALL THIS MUST RETURN
        all = dom_seas_all[:,0,0][i::4].tolist()
        base = dom_seas_base[:,0,0][i::4].tolist()
        ## Now starts from DJF not MAM
        now = dom_seas_now[:,0,0][(i+1 if i+1<len(axes.flatten()) else 0)::4].tolist()
        angle = 60
        textprops = {'color': 'w','rotation' : 20 if i == 0 else 5, 'fontsize': 35}
        print(all,all.count(0), all.count(1))
        raise KeyboardInterrupt
        if i!= 1: # JJA
            ax.pie([all.count(0), all.count(1)],# labels = ['Frequency', 'Intensity'],
                autopct='%1.1f%%', colors = ['#bd004e', '#142354'], startangle=angle,
                radius=1.3,pctdistance = 0.85, textprops = textprops,
                wedgeprops=dict(width=0.4, edgecolor = 'w', linewidth = 10))
            ax.pie([now.count(0), now.count(1)],# labels = ['Frequency', 'Intensity'],
                autopct='%1.1f%%',pctdistance = 0.65, colors = ['#bd004e', '#142354'], startangle=angle,
                radius=0.6,textprops = textprops,
                wedgeprops=dict(width=0.35,edgecolor = 'w', linewidth = 10))
        else:
            ax.pie([all.count(0), all.count(1)],# labels = ['Frequency', 'Intensity'],
                colors = ['#bd004e', '#142354'], startangle=angle,radius=1.3,
                #autopct='%1.1f%%',pctdistance = 0.85, textprops = {'color': 'w', 'rotation' : 30, 'fontsize': 35},
                wedgeprops=dict(width=0.4, edgecolor = 'w', linewidth = 10))
            ax.pie([now.count(0), now.count(1)],# labels = ['Frequency', 'Intensity'],
                colors = ['#bd004e', '#142354'], startangle=angle, radius=0.6,
                #autopct='%1.1f%%',pctdistance = 0.6, textprops = textprops,
                wedgeprops=dict(width=0.35,edgecolor = 'w', linewidth = 10))
        ax.pie([base.count(0), base.count(1)],# labels = ['Frequency', 'Intensity'],
               colors = ['#bd004e', '#142354'], startangle=angle,
               radius=0.9,autopct='%1.1f%%', pctdistance = 0.81,textprops = textprops,
               wedgeprops=dict(width=0.3,edgecolor = 'w', linewidth = 10))
        
        ax.text(0.1, 1, seas[i], fontsize = 45, weight = 'heavy', ha = 'center', va = 'center', transform = ax.transAxes)
        
    fig.tight_layout()
    fig.savefig(os.path.join(img_wd, "ME_dom_pie.png"), bbox_inches = 'tight', dpi = 300)
#endregion
