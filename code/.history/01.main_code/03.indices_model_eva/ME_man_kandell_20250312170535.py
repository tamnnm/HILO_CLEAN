...
def point_map(par:Optional[Literal['R','T2m']] = 'R',dataset:Optional[Literal['rean', 'obs']]= 'rean', direction = 'vertical'):
    ...
    for ax in axs:
        ...
        if ax.get_subplotspec().get_geometry()[1] != 1 \
            and ((ax.get_subplotspec().rowspan.start == no_row - 1 and ax.get_subplotspec().colspan.start < no_row * no_col - len(var_tuple))
                 or
                 (ax.get_subplotspec().rowspan.start == no_row - 1 and ax.get_subplotspec().colspan.start == no_col - 1)):
        
            continue
        
        # For the R plot, skip the first column in the every row except the first one
            #? ax.get_subplotspec().get_geometry()[1] != 1 => Check if this is not the island plot
        if par == 'R' \
            and ax.get_subplotspec().colspan.start > 0 \
