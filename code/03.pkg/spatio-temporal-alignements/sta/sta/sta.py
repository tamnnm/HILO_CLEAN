...
def sta_distances(x, y, metric, beta=0.01, epsilon=0.01,
    ...
    if isinstance(x, np.ndarray):
        if x.dtype == object:
            x = nested_tensor([torch.tensor(arr, dtype=dtype, device=device) for arr in x], \
    ...
    #** TESTING
    if isinstance(metric, np.ndarray) and metric.dtype ==object:
        try:
            metric = nested_tensor([torch.tensor(arr,dtype=dtype, device=device) for arr in metric], \
            dtype=dtype, device=device)
        except Exception as e:
            raise ValueError("Metric should be a nested tensor or a 2D tensor.")
    else:
