    ...
    def compute(self):
        ...
        if self.amari is not None:
            ...
            if Ksum.ndimension() == 2:
                ...
                # TODO: Optimize for nested tensor K (now not yet)
...
def _compute_divergence(self, x_chunk, y_chunk):
    """
    Compute divergence between chunks of data.

    Parameters
    ----------
    x_chunk: torch.Tensor
        Chunk of first time series data
    y_chunk: torch.Tensor
        Chunk of second time series data
    ...
    G: torch.Tensor
        Gradient information (if return_grad=True)
    Kb: torch.Tensor
        Kernel matrix (if warmstart=True)
    """
    if self.amari is not None:
        Ksum = self.amari ** self.ot_params["gamma"]
        if Ksum.ndimension() == 2:
            ytild = convol_imgs(y_chunk / Ksum[None, :], self.K)
        else:
            ytild = (self.K / Ksum[:, None]).mm(y_chunk.t()).t()

        normalize = self.fyky is None
        ww, G, Kb, fyky = amarikl(
            x_chunk, y_chunk, ytild, self.K,
            normalize=normalize,
            compute_grad=self.return_grad,
            Kb=self.fyky,
            **self.ot_params
        ...
        ww, G, Kb, axx, ayy = divergencekl(
            x_chunk, y_chunk, self.K,
            compute_grad=self.return_grad,
            Kb=self.Kb, axx=self.axx,
            **self.ot_params
        )
