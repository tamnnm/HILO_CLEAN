...
=======
def MFC_calc(
    SH: Union[np.ndarray, xr.DataArray], # Specific humidity
...
<<<<<<< HEAD
    P_name = find_name(P, 'level')
    
    vapour_mass = SH * delta_P/g
...
=======
    ...
    SH_dim_order = list(SH.dims)
    layer_mass = layer_mass.transpose(*SH_dim_order)
    ...
    dq_dx = gradient_ds(int_vapour_mass, 'lon')
    dq_dy = gradient_ds(int_vapour_mass, 'lat')
