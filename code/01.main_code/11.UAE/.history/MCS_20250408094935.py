...
def water_vapour_mass_absolute(
    ...
    if isinstance(SH, xr.DataArray):
        MFC = - U * gradient_ds(SH, 'lon') - V * gradient_ds(SH, 'lat') - SH * (gradient_ds(U, 'lon') + gradient_ds(V, 'lat'))
    else:
        MFC = - U * gradient_ds(SH, 0) - V * gradient_ds(SH, 'lat') - SH * (gradient_ds(U, 'lon') + gradient_ds(V, 'lat'))
