...
=======
    ...
    if not os.path.exists(f"clean_{ind}"):
        os.makedirs(f"clean_{ind}")
>>>>>>> c80f4457 (First commit)
...
=======
def delta_gev(ind = 'Tx'):
    
    T_flag = True if "T" in ind else False
    JJA_name = 'MST'
    markers = ['o','s','^','v']#,'<','>']
    # edgecolors = ['black','red','blue','green','yellow']#,'purple']
    indice_list =['DT_20','DT_50',f'DT_{JJA_name}']
    if T_flag:
        indice_plot_list = [
            r'$\Delta T_{20\mathrm{yr} \rightarrow 1\mathrm{yr}}$',
            r'$\Delta T_{50\mathrm{yr} \rightarrow 1\mathrm{yr}}$',
        ...
        indice_plot_list = [
            r'$\Delta R_{20\mathrm{yr} \rightarrow 1\mathrm{yr}}$',
            r'$\Delta R_{50\mathrm{yr} \rightarrow 1\mathrm{yr}}$',
            r'$\Delta R_{1\mathrm{yr} \rightarrow \mathrm{MSR}}$'
        ]
>>>>>>> c80f4457 (First commit)
...
=======
    delta_cmap=sns.color_palette("YlOrRd", as_cmap=True) if T_flag else sns.color_palette("BuPu", as_cmap=True)

    min_20, max_20, min_50, max_50, min_shape, max_shape = 200, 0, 200, 0, -0.5, 0.5
    for i in range(12):
        i_ri = i+1
        # Run fit check
        og_csv_name = f"gev_{ind}/OG_{i}.csv"
        csv_name = f"gev_{ind}/OG_DT_{i}.csv"
        if os.path.exists(og_csv_name):
            data = pd.read_csv(csv_name)
        else:
            # data = fit_gev("Tx", og=True, resample_mode=i)
            org_data = pd.read_csv(og_csv_name)
>>>>>>> c80f4457 (First commit)
            ri = 1/(i_ri)
...
=======
            org_data[f'DT_{JJA_name}'] = x_Tr(1, org_data['loc'], org_data['scale'], org_data['shape'], ri=ri) - org_data[JJA_name]
            data = pd.concat([org_data['name_station'], org_data['loc'], org_data['scale'], org_data['shape'], org_data['DT_20'], org_data['DT_50'], org_data[f'DT_{JJA_name}']], axis=1)
            data.to_csv(csv_name)
...
<<<<<<< HEAD
                    sc = ax.scatter(-valid_data['shape'], valid_data[indice_list[j]], c=valid_data['scale'], marker=markers[order], cmap='Spectral', s=250, edgecolors='k')
=======
                    sc = ax.scatter(-valid_data['shape'], valid_data[indice_list[j]], c=valid_data['scale'], marker=markers[order], cmap=delta_cmap, s=250, edgecolors='k', linewidths=10 if i_ri == 1 else 1, zorder=1000 if i_ri == 1 else 0)
>>>>>>> c80f4457 (First commit)
                else:
...
=======
            ax.set_ylabel(r'$\Delta T(^\circ C)$') if T_flag else ax.set_ylabel(r'$\Delta R(mm)$')
    #! The outlier dude is VIETTRI
...
<<<<<<< HEAD
    ...
    with multiprocessing.Pool() as pool:
=======
        result = root_scalar(lambda sig: DTi_20[i, j] - DT(20, mu, sig * (sig > 0) + 1e-6, K_20[i, j]), x0=1.0).root
        result_50 = root_scalar(lambda sig: DTi_50[i, j] - DT(50, mu, sig * (sig > 0) + 1e-6, K_50[i, j]), x0=1.0).root
        return (i, j, result, result_50)

    indices = list(np.ndindex(K_20.shape))
    #? Use multiprocess instead of multiproccessing => avoid "Can't pickle local object" due to nested function
...
=======
            levels = np.arange(0.8, 5, 0.8)  # starts from 0.8
            CS = ax.contour(-K_20,DTi_20,SIG_20,levels = levels, colors='black',linestyles='dashed')
            ax.clabel(CS, inline=1, fontsize=35, fmt=custom_fmt)
        if indice_list[j] == 'DT_50':
            levels = np.arange(0.8, 5, 0.8)  # same for DT_50
            CS = ax.contour(-K_50,DTi_50,SIG_50,levels = levels, colors='black',linestyles='dashed')
>>>>>>> c80f4457 (First commit)
            ax.clabel(CS, inline=1, fontsize=35, fmt=custom_fmt)
...
=======
    cbar.set_label(r'$\sigma (^\circ C)$' if T_flag else r'$\sigma (mm)$', rotation=0, fontsize=40, labelpad=3)

    fig.savefig(os.getenv('img')+f"/Delta_GEV_{ind}.png", bbox_inches='tight', transparent=True)
    # axes = axes.flatten()
    plt.close(fig)
>>>>>>> c80f4457 (First commit)
#endregion
...
def cluster_plot(resample_mode = 3):
    params ={
        ...
        'axes.labelsize': 50,
=======
def cluster_plot(ind='Tx', resample_mode = 3):
    params ={
        'xtick.labelsize': 45,
        'ytick.labelsize': 45,
        'axes.labelsize': 60,
...
=======
    data = pd.read_csv(f"gev_{ind}/OG_{resample_mode}.csv")
    fig = plt.figure(figsize=(55, 30))
    gs = fig.add_gridspec(1, 2, width_ratios=[9,1], wspace = -0.7)
>>>>>>> c80f4457 (First commit)
...
=======
    ax_1.text(112, 10, "Spratly Islands", fontsize=40, rotation=45)
    ax_1.text(111, 15, "Paracel Islands", fontsize=40, rotation=45)
    ax_1.text(110, 12, "E  a  s  t   S  e  a", fontsize=50, rotation = 90, fontstretch = 'expanded', alpha = 0.75)
>>>>>>> c80f4457 (First commit)
...
#region - ODD OF EXCEEDING
def doubling(resample_mode, ind='Tx', gev_fit=False, doubling: Optional[Literal['linear', 'log', 'both']] = 'linear'):
>>>>>>> c80f4457 (First commit)
    """_summary_
...
=======
    with h5py.File(f'clean_{ind}/OG_{resample_mode}.h5', 'r') as f:
        for key,item in f.items():
            raw_data_dict[key] = item[:]
    if os.path.exists(f"gev_{ind}/OG_{resample_mode}.csv"):
        data_fitted_df = pd.read_csv(f"gev_{ind}/OG_{resample_mode}.csv")
>>>>>>> c80f4457 (First commit)
        if resample_mode == 0:
...
=======
        data_fitted_df = fit_gev(ind, og=True, resample_mode=0)
>>>>>>> c80f4457 (First commit)
...
if __name__ == '__main__':
    ...
    # !! R is simply desaster, look into it later
    # for i in range(13):
    #     fit_gev('R', og=True, resample_mode=i)
    # delta_gev(ind='Tx')
    # fit_gev('Tn', og=True, resample_mode=0)
    # cluster_plot(resample_mode=0)
    doubling(resample_mode=0, gev_fit=False, doubling='log')
    # doubling(resample_mode=3, gev_fit=False, doubling='log')
>>>>>>> c80f4457 (First commit)
