...
class DillPool(multiprocessing.pool.Pool):
    ...
    def _reduce_tasks(self, tasks):
        return dill.dumps(tasks)
...
def delta_gev():
    ...
    def dt_function_20(sig, i, j):
        # This is what was inside the first lambda
        return DTi_20[i, j] - DT(20, mu, sig * (sig > 0) + 1e-6, K_20[i, j])
    
    def dt_function_50(sig, i, j):
        # This is what was inside the second lambda
        return DTi_50[i, j] - DT(50, mu, sig * (sig > 0) + 1e-6, K_50[i, j])
    
    def compute_SIG(args):
        i, j = args
        result = root_scalar(lambda sig: dt_function_20(sig, i, j), bracket=[1e-5, 1000]).root
        result_50 = root_scalar(lambda sig: dt_function_50(sig, i, j), bracket=[1e-5, 1000]).root
        return (i, j, result, result_50)

    indices = list(np.ndindex(K_20.shape))
    
    # Use dill as the serializer for nested function
    multiprocessing.set_start_method('spawn', force=True)  # Optional but recommended
    
    with multiprocessing.Pool(initializer=lambda: setattr(multiprocessing.current_process(), '_pickler', dill)) as pool:
    # with con_fu.ProcessPoolExecutor() as executor:
        # results = list(executor.map(compute_SIG, indices))
