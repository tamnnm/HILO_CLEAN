...
def delta_gev(ind = 'Tx'):
    ...
    T_flag = True if ind in temp_tuple else False
    JJA_name = 'MST'
    markers = ['o','s','^','v']#,'<','>']
    ...
    for i in range(12):
        ...
        if i > 1:
            ...
            print(data['DT_50'].max(),data['DT_20'].max(), data['DT_20'].min(), data['DT_50'].min())
        ...
        for j,ax in enumerate(axes.flatten()):
            if i_ri%3 == 0: # or i_ri == 1:#(i_ri == 1 and j%2 != 0):
                ...
                if 'MST' in indice_list[j]:
                    sc = ax.scatter(-valid_data['shape'], valid_data[indice_list[j]], c=valid_data['scale'], marker=markers[order], cmap=delta_cmap, s=250, edgecolors='k')
                else:
    ...
    def compute_SIG(args):
        i, j = args
        result = root_scalar(lambda sig: DTi_20[i, j] - DT(20, mu, sig * (sig > 0) + 1e-6, K_20[i, j]), bracket=[1e-10, 1e4]).root
        result_50 = root_scalar(lambda sig: DTi_50[i, j] - DT(50, mu, sig * (sig > 0) + 1e-6, K_50[i, j]), bracket=[1e-10, 1e4]).root
