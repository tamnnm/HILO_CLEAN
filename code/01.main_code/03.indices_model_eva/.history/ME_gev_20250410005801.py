...
def delta_gev(ind = 'Tx'):
    ...
    def compute_SIG(args):
        i, j = args
        result = root_scalar(lambda sig: DTi_20[i, j] - DT(20, mu, sig * (sig > 0) + 1e-6, K_20[i, j]), bracket=[1e-10, 1e10]).root
        result_50 = root_scalar(lambda sig: DTi_50[i, j] - DT(50, mu, sig * (sig > 0) + 1e-6, K_50[i, j]), bracket=[1e-10, 1e20]).root
