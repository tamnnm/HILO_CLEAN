...
class DeltaGEV:
    def __init__(self):
        self.markers = ['o', 's', '^', 'v']
        self.indice_list = ['DT_20', 'DT_50', 'DT_MST']
        self.indice_plot_list = [
        ...
        # Initialize these in your setup method or where you have the data
        self.mu = 10  # Set your actual value
        self.DTi_20 = None
        self.DTi_50 = None
        self.K_20 = None
        self.K_50 = None
    ...
    def setup_data(self, min_20, max_20, min_50, max_50, min_shape, max_shape):
        k = np.linspace(min_shape, max_shape, 100)
        DTi_20 = np.linspace(min_20, max_20, 100)
        DTi_50 = np.linspace(min_50, max_50, 100)
        
        self.K_20, self.DTi_20 = np.meshgrid(k, DTi_20)
        self.K_50, self.DTi_50 = np.meshgrid(k, DTi_50)
        self.SIG_20 = np.zeros(self.K_20.shape)
        self.SIG_50 = np.zeros(self.K_50.shape)

    def dt_function_20(self, sig, i, j):
        return self.DTi_20[i, j] - DT(20, self.mu, sig * (sig > 0) + 1e-6, self.K_20[i, j])
    
    def dt_function_50(self, sig, i, j):
        return self.DTi_50[i, j] - DT(50, self.mu, sig * (sig > 0) + 1e-6, self.K_50[i, j])
    
    def compute_SIG(self, args):
        i, j = args
        result_20 = root_scalar(lambda sig: self.dt_function_20(sig, i, j),
                           bracket=[1e-5, 1000]).root
        result_50 = root_scalar(lambda sig: self.dt_function_50(sig, i, j),
                              bracket=[1e-5, 1000]).root
        return (i, j, result_20, result_50)
    
    def process_data(self):
        indices = list(np.ndindex(self.K_20.shape))
        
        with ProcessPoolExecutor() as executor:
            results = list(executor.map(self.compute_SIG, indices))

        for i, j, sig_20, sig_50 in results:
            self.SIG_20[i, j] = sig_20
            self.SIG_50[i, j] = sig_50
    
    def run(self):
        indice_list
        # Your main plotting and processing logic here
        ...
        # Your existing loop and plotting code here
        for i in range(12):
            i_ri = i+1
            # Run fit check
            ...
            for j,ax in enumerate(axes.flatten()):
                if i_ri%3 == 0: # or i_ri == 1:#(i_ri == 1 and j%2 != 0):
                    order = int(i_ri/3) - 1
                    valid_data = data.dropna(subset=[self.indice_list[j]])
                    if 'MST' in self.indice_list[j]:
                        ...
                        max_value = max_20 if '20' in self.indice_list[j] else max_50
            ...
            # Set the title inside the graphing area
                ax.text(0.05, 1.1, self.indice_plot_list[j], transform=ax.transAxes,
        ...
        self.setup_data(min_20, max_20, min_50, max_50, min_shape, max_shape)
