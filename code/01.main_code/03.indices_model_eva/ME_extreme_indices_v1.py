...
def gen_base(var:Optional[Literal["tp","max","min","vwnd","uwnd"]],start:Optional[int],end:Optional[int],dataset:Optional[Literal["cera","era","era5","noaa"]],percentile:list = None,window='5'):
    #create base file
    var_no=param[var]
    fol=f'para_{var_no}/'
    ...
    if os.path.exist(ofile):
        ...
        if var_no==228 and dataset != "obs":
            ...
            cdo.selyear(time_range,input=full_ifile,output=full_ofile,options='L')
        if var in param['extreme'].keys():
            if percentile:
                ...
                for rp in percentile:
                    ...
                    pminfile=full_name(f'b{rp}p_min_{ifile}')
                    pmaxfile=full_name(f'b{rp}p_max_{ifile}')
                    if not check_exist(opfile):
                        ...
                        cdo.seldate(f'{start}/{end}', input=f'-ydrunpctl {rp},{window},pm=r8 {ibfile} {pminfile} {pmaxfile}',output=opfile)
    return
...
def get_ifiles(ind, dataset):
    ...
    fol = f'para_{var_no}/'
    fmin=f'min_{dataset}_{var_no}.nc'
    fmax=f'max_{dataset}_{var_no}.nc'
...
class gen_ind:
    ...
    def __call__(self,ind,dataset):
        ...
        self.ifile=self.fol+(iminfile or imaxfile)
        ...
        if pctl_ind is not None:
            self.ibfile=self.fol+f'b{pctl_ind}p_{self.ifile}'
...
gen_base(var='tp',start=1961,end=1990,dataset='cera',percentile=[95,99],window='5')
