...
import sys
from .ME_extreme_indices import Data_wd, Data_nc_2019
from .ME_man_kandell import no_R_station, name_R_station
>>>>>>> c80f4457 (First commit)
#endregion
...
=======
obs_ds = xr.decode_cf(xr.open_dataset( os.path.join(Data_nc_2019, 'OG_R_daily_1960_2019.nc')))
...
def process_station(option,no_station, pcp_subset, no_wet_subset, sdii_subset):
    # ? Skip the first DJF value for "now" period
>>>>>>> c80f4457 (First commit)
...
<<<<<<< HEAD
=======
        #? i::4 => get every 4th value starting from index i => Get the seasonal data
>>>>>>> c80f4457 (First commit)
...
=======
    ...
    axes = []
    for i in range(4):
        # Left subplot - normal projection
        ax_left = fig.add_subplot(gs[i, 0])
    
        # Right subplot - polar projection
        ax_right = fig.add_subplot(gs[i, 1], projection='polar')
        ax_right.axis('off')
...
<<<<<<< HEAD
        ...
        w,l,p=ax.pie([all.count(0), all.count(1)],# labels = ['Frequency', 'Intensity'],
            autopct='%1.1f%%', colors = ['#D35589', '#626C8D'], startangle=angle,
        ...
        w,l,p=ax.pie([now.count(0), now.count(1)],# labels = ['Frequency', 'Intensity'],
            autopct='%1.1f%%',pctdistance = 0.78, colors = ['#C82A6C', '#3B4871'], startangle=angle,
...
=======
        total = all.count(0) + all.count(1)
        
        sn_max = max(dom_seas_all[:,i,1])
        si_max = max(dom_seas_all[:,i,2])
        
        angle = 90
        
        int_colors = ['#c7eae9', '#98dad9', '#68c7c8']
        freq_colors = ['#fbe499', '#f9d55c', '#f7c41f']
        
        # Scatter plot
        for j, no_station in enumerate(no_R_station):
            s_n = dom_seas_all[j,i,1]
            s_i = dom_seas_all[j,i,2]
            dom = dom_seas_all[j,i,0]
            print(s_n, s_i)
            if dom == 0:
                ax_left.scatter(s_n, s_i, marker = '8', s = 300, facecolors=freq_colors[-1], alpha = 0.75)
            else:
                ax_left.scatter(s_n, s_i, marker = 'X', s = 300, facecolors=int_colors[-1], alpha = 0.75)
                
        # dom_dict[seas[i]] = dom_seas
        
        # Plot the line
        ax_left.plot([0,max(si_max,sn_max)+30],[0,max(si_max,sn_max)+30], color = 'k', linestyle = '--')
        ax_left.axhline(0, color='black', linewidth=0.5)
        ax_left.axvline(0, color='black', linewidth=0.5)
        ax_left.set_xlim(0,sn_max+30)
        ax_left.set_ylim(0,si_max+30)
        ...
        # # raise KeyboardInterrupt
        # w,l,p=ax.pie([all.count(0), all.count(1)],# labels = ['Frequency', 'Intensity'],
        #     autopct='%1.1f%%', pctdistance = 0.85, colors = ['#fbe499', '#c7eae9'], startangle=angle, counterclock=False,
        #     radius=1.5, textprops = textprops,
        #     wedgeprops=dict(width=0.9, edgecolor = 'w', linewidth = 10))
        ...
        #     radius=1.1,textprops = textprops,counterclock=False,
        #     wedgeprops=dict(width=0.85,edgecolor = 'w', linewidth = 10))
        ...
        # w,l,p=ax.pie([base.count(0), base.count(1)],# labels = ['Frequency', 'Intensity'],
        #        autopct='%1.1f%%', pctdistance = 0.65, colors = ['#f7c41f', '#68c7c8'], startangle=angle,
        #        radius=0.65,textprops = textprops,counterclock=False,
        #        wedgeprops=dict(width=0.4,edgecolor = 'w', linewidth = 10))
        ...
        ax_right.spines.clear()
        freq_start = angle*2*pi/360 # The start of the pie chart
        text_start = 3/4*2*pi
        xs_freq = [(i*pi*2)/total for i in [base.count(0), now.count(0), all.count(0)]]
        xs_int = [(i*pi*2)/total for i in [base.count(1), now.count(1), all.count(1)]]
        int_count = [all.count(1), now.count(1), base.count(1)]
        ys = [2,3.5,5]
        
        for j, y in enumerate(ys):
            ax_right.barh(y, -xs_freq[j], left = freq_start, height=1.2, color=freq_colors[j])
            ax_right.scatter(freq_start-xs_freq[j], y, s=2800, color=freq_colors[j], zorder=2)
            ax_right.scatter(freq_start, y, s=2800, color=freq_colors[j], zorder=2)
            ax_right.barh(y, -xs_int[j], left=freq_start-xs_freq[j], height=1.2, color=int_colors[j])
            # ax_right.scatter(freq_start-xs_freq[j]-0.6, ys[j], s=5000, color=freq_colors[j], zorder=2)
            # ax_right.scatter(freq_start+0.6, ys[j], s=5000, color=freq_colors[j], zorder=2)
            
            ax_right.text(text_start,y, f'{int_count[j]/total*100:.1f}%', ha='center', va='center', fontsize=40, color='k', transform = ax_right.transData)
        # ax_right.text(0, 0.95, seas[i], fontsize = 70, fontweight = 'bold', ha = 'center', va = 'center', transform = ax_right.transAxes)
        
    fig.savefig(os.path.join(img_wd, "ME_dom_pie_test.png"), bbox_inches = 'tight', dpi = 400, transparent=True)
>>>>>>> c80f4457 (First commit)
#endregion
