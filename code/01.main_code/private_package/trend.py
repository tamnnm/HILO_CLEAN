...
def mann_kendall(actual, alpha=0.05, method: Optional[Literal["Seasonal", "PW", "no-trend PW", "Seasonal PW", "Sens slope"]]=None):
    ...
    methods={
        ...
        "Sens slope": mk.sens_slope,
    }
    if method == "Sens slope":
        return methods[method](check_dim(actual)).slope
    else:
        return methods[method](check_dim(actual), alpha=alpha).slope
...
def DISO(actual, predicted,check = False):
    """ DISO index (2007)"""
    actual,predicted=non_nan(check_dim(actual),check_dim(predicted))
    if all(element == 0 for element in actual):
        print("check actual")
    ...
    nme=_nme(actual, predicted)
    nrmse=_nrmse(actual, predicted)
    DISO=np.sqrt((R-1)**2+nme**2+nrmse**2)
    return DISO
...
METRICS_SINGULAR = {
    ...
    'mandall_kendall': {'function': mann_kendall, 'full_name': "Mann-Kendall Test"},
}
...
def evaluate_single(actual, metrics):
    ...
    for name in metrics:
        try:
            ...
            results = np.nan
            print('Unable tohel compute metric {0}: {1}'.format(name, err))
    return results
