...
cmap = plt.get_cmap('plasma', len(TC_rank))
...
fig, ax = ini_plot(dlat, ulat, dlon, ulon, pad=5,
                   figsize=[12, 7], label_size=10, title="Track of TC in the South East Asia region", grid=True)
if False:
    ...
    # * Set up the plot
    make_cbar(ax, cmap, norm, range(len(TC_rank)-1), TC_rank[1:])
    # * Track density
    # Save the figure
    # Adjust layout to fit the figure tightly
    fig.tight_layout()
    fig.savefig("Track_TC_SEA_cat.png")
    plt.clf()
...
fig, ax = ini_plot(dlat, ulat, dlon, ulon, pad=5,
                   figsize=[12, 7], label_size=10, title="Density of TC in the South East Asia region")
...
for i, rank in enumerate(TC_rank):
    ...
    fig, ax = ini_plot(dlat, ulat, dlon, ulon, pad=5,
                       figsize=[12, 7], label_size=10, title=f"Density of TC - level:{TC_rank[i]} in the South East Asia region")
    c = ax.pcolormesh(lon_grid, lat_grid, annual_TC_density,
    ...
    make_cbar(ax, cmap="winter", sm=c, custom_cbar=False)
    fig.tight_layout()
    fig.savefig(f"TC_SEA_density_TC_rank{i}.png")
    plt.clf()
...
for i, rank_label in enumerate(TC_rank[1:]):  # Skip the first category
    ax.bar(year_range, normalized_counts[i],
           label=rank_label, bottom=bottom, color=cat_colors[i])
...
ax.set_ylabel("Percentage of TC")
ax.set_title("Categorized TC in the South East Asia region", pad=20)
ax2.set_ylabel("Total Count of TC")

fig.savefig("TC_SEA_year.png")
