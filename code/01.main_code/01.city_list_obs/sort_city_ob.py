import os
os.chdir('/work/users/tamnnm/code/01.main_code/01.city_list_obs/')
...
def f_write(list_city_info, file_name):
    with open(f'city/{file_name}.txt', 'w') as file:
        ...
        # => crucial for no_station in netcdf
        for city in sorted(list_city_info, key=lambda x: (x[3], x[4])):
            line = ','.join(city)+'\n'
            file.write(line)
    return

# print(len(rain_city),len(temp_city),len(uwnd_city),len(vwnd_city))
