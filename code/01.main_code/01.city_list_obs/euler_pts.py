...
for file_path in Data_name:
    ...
    # MUST: add values or else it appears as a dataarray subset
        reso_lat = abs(lat_all[1]-lat_all[0])
        reso_lon = abs(lon_all[1]-lon_all[0])
        print(reso_lat, reso_lon)
        city_name_list = []

        # --------------------- List of recorded cities -------------------- #
        #"""
        euleur_file =Code_wd+file_path+"_euler_pts.txt"
        ...
        for city_list_path in glob.glob(Code_wd+"city/*.txt"):
            with open(city_list_path, 'r') as f:
                lines = f.readlines()
                f.close()

            # ---- transform line into list and go through each line to get information --- #
            all_city = []
            for i, line in enumerate(lines):
                coor_city = []
                ...
                if city_name in city_name_list:
                    ...
                    #max_distance = reso_lat**2 + reso_lon**2
                    for lat_test in lat_all:
                        for lon_test in lon_all:
                            if (abs(lat_test-lat_target) > reso_lat) or (abs(lon_test-lon_target) > reso_lon) :
                                continue
                            else:
                    ...
                    print(len(distance),lat_t,lat_target, lon_target)
                    #break
                    # -------------- Find the indices of the closest point ------------- #
                    if len(distance) > 4 or len(distance) < 1:
                        #raise KeyError("Invalid number of points")
