...
# ------------------------ Constant variable ----------------------- #
# region
pdata = "/work/users/tamnnm/Data/pap25_QA_1945/"
pdata_grib = "$wrfdata/gribfile/era5_6h_1940_2023/"
pdata_grib_synop = "$wrfdata/netcdf/era5_synop_1940_2023/"
pdata_real = pdata +"Filter_Data/"
pdata_ob = pdata + 'ob/'
pdata_cal = pdata + 'cal/'
pdata_final = pdata + "temp_run/"
...
# region
city_path = "City.csv"
city_list = pd.read_csv(f'{pdata}{city_path}')
...
class StationData:
    def __init__(self, station_nc_ls, pdata):
        ...
        if len(no_year)*12 < len(dataset_csv.index):
            ...
            for year in no_year:
                for month in range(1, 13):
                    time_dims.append(f'19{int(year)}-{month:02d}-01')
                    #len_ds += 1
    ...
    def create_station_nc(self): #_ls, pdata, city_list):
        for station_nc in self.station_nc_ls:
            odir = Path(self.pdata_real,station_nc,".nc")
            tail = station_nc[-2:]
            ...
            if odir.exists():
                ...
                for name in Path(pdata_real).iterdir():
...
def ds_open(folder_name):
    ...
    def ds_var_name(filename, ds_name, var_left=None, var_skip=None):
        ...
        if ds_name == "GHCN":
            ...
            ds_dir_final = pdata_final+name_final+'.nc'
            ...
            if var_skip is not None:
                ...
                for i, var_name_uniform in enumerate(var_left):
                    ...
                    ds_var.to_netcdf(pdata_final+name_final+".nc", 'w', engine="h5netcdf",
                                     format='NETCDF4')
            ...
            for ds_name_file in os.listdir(f'{folder_name}{ds_name}'):
                if ds_name_file.endswith(".nc"):
                    ...
                    for i, var_name_uniform in enumerate(var_name_uniform_list):
                        ds_final_path = f'{pdata_final}{ds_name}_{var_name_uniform}.nc'
...
def call_check_dataset(filename, type_option=None, open=None, check=None):
    # if os.path.exists(pdata_grib+filename):
    filename = f'{pdata_grib_synop+filename}.nc'
    print(filename)
    if os.path.exists(filename) is True:
        filename = filename
    else:
        filename = f'{pdata_grib+filename}_even.nc'
...
def grib_group(var_grib, var_grib_uniform=None):
    ds_final_dir = f'{pdata_final}era5_{var_grib_uniform}.nc'
    ds_final_dir_wrf = f'{pdata_final}era5_{var_grib_uniform}_wrf.nc'
    ds_merge_dir = f'{pdata_final}era5_{var_grib_uniform}_merge.nc'
...
for i, var_uniform in enumerate(temp_var+prep_var):
    ds_final_dir = pdata_final+"wrf_"+var_uniform+".nc"
    if os.path.exists(ds_final_dir):
...
for i, var_name_dir in enumerate(var_left):
    ...
    try:
        ...
        print("No projection")
    ds_var.to_netcdf(f'{pdata_final}wrf_RAINC.nc', 'w', engine="h5netcdf",
                     format='NETCDF4')
    ...
    try:
        ...
        try:
            ...
            print("No projection")
        ds_var_2.to_netcdf(f'{pdata_final}wrf_RAINNC.nc', 'w', engine="h5netcdf",
                           format='NETCDF4')
    ...
    # raise KeyError
    ds_var.to_netcdf(f'{pdata_final}wrf_{var_uniform}.nc', 'w', engine="h5netcdf",
                     format='NETCDF4')
...
for var_name in (["tmp", "precip"]):
    ...
    for i, city_name in enumerate(city_good):
        ...
        for i, folder in enumerate(folder_plot):
            ...
            ds_mean_city = f'{pdata_final}{folder}_{var_name}_{city_name}.nc'
    ...
    # labs = [l.get_label() for l in lines]
        ...
        # plt.savefig(f'{pdata}/Data/test/{ct}.jpg',format="jpg")
        plt.savefig(
            f'{pdata}pic/final/{var_name}_{city_name}.jpg', format="jpg", dpi=500)
        # plt.legend()
        plt.tight_layout()
        plt.close()
    ...
    # Plot wrf and insitu data
        ...
        ds_wrf_city = f'{pdata_final}wrf_{var_name}_{city_name}.nc'
        ...
        plt.tight_layout()
        plt.savefig(
            f'{pdata}pic/final/wrf_{var_name}_{city_name}.jpg', format="jpg", dpi=500)
        #                                      color='r', label="Downscaled ERA5", marker='o')
"""
else:
    for i in range(len(folder)):
        ...
        try:
            ...
            plt.tick_params(axis='x', direction='in')
            # plt.savefig(f'{pdata}/Data/test/{ct}.jpg',format="jpg")
            plt.savefig(os.path.join(pdata, "Data", fold,
...
for b in range(len(all_para)):
    for a in range(len(pre)):
        ...
        if all_para[b] in temp_var:
            folder=f'{pdata}Data/REAL/T_pic/{all_para[b]}'
            name=temp_name[a][:-2]
            c=r'$T^o$'
            year_plot=city_T_year
        else:
            folder=f'{pdata}Data/REAL/P_pic/{all_para[b]}'
            name=prep_name[a][:-2]
