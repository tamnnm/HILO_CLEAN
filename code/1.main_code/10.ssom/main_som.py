...
os.chdir("/data/projects/REMOSAT/tamnnm/wrf_data/netcdf/noaa/noaa_synop_1877_2015/full_nc")
var = 'air'  # mean sea level pressure
ifile = 'noaa_167_1881.nc'  # input file in netcdf format

# reading and processing input data
d1 = xr.open_dataset(ifile)[var]
d1.lon.values[d1.lon.values <
              0] = d1.lon.values[d1.lon.values < 0] + 360
d1 = d1.isel(lat=slice(0, 65), lon=slice(19, 91))
lat2d, lon2d = d1['lat'].values, d1['lon'].values
# grouping by seasons
...
for key in ['MAM', 'SON', 'DJF', 'JJA'][2:3]:
    ...
    if not os.path.exists(odir):
        os.makedirs(odir)
    # write original data for latter checking if necessary
    # d3.to_netcdf('output/'+key+'/orgdata.nc')
    print(d3)
