# %%
# -------------------------- IMPORT MODULE ------------------------- #

import cartopy.crs as ccrs
import matplotlib
import matplotlib.pyplot as plt
<<<<<<< HEAD
import cartopy.feature as cfeature
from cartopy.io.shapereader import Reader
from cartopy.feature import ShapelyFeature
import cartopy.feature as cfeature
from cartopy.mpl.ticker import LongitudeFormatter, LatitudeFormatter
=======
>>>>>>> c80f4457 (First commit)
# import metpy.calc as mpcalc
# from metpy.units import units
import numpy as np
import pandas as pd
import shapefile as shp
import os
<<<<<<< HEAD
from HersheyFonts import HersheyFonts
from mpl_toolkits.axes_grid1.inset_locator import inset_axes

# %%
params = {
    'axes.labelsize': 5,
    'font.size': 5,
    'text.usetex': True,
    # 'font.family':'monospace',
    'legend.fontsize': 5,
    'legend.loc': 'upper right',
    'legend.labelspacing': 0.25,
    'xtick.labelsize': 7,
    'ytick.labelsize': 7,
    'lines.linewidth': 4,
    'text.usetex': False,
    # 'figure.autolayout': True,
    'ytick.right': True,
    'xtick.top': True,

    'figure.figsize': [10, 10],  # instead of 4.5, 4.5
    'axes.linewidth': 1.5,

    'xtick.major.size': 2,
    'ytick.major.size': 2,
    'xtick.minor.size': 2,
    'ytick.minor.size': 2,

    'xtick.major.width': 2,
    'ytick.major.width': 2,
    'xtick.minor.width': 2,
    'ytick.minor.width': 2,

    'xtick.major.pad': 5,
    'ytick.major.pad': 5,
=======
import xarray as xr
from my_junk import plot_shp
import matplotlib.lines as mlines

# %%
params = {
    'axes.titlesize': 40,
    'axes.labelsize': 10,
    'axes.labelpad': 15,
    'font.size': 10,
    'font.family': 'cmss10',
    'mathtext.fontset': 'stixsans',
    'legend.fontsize': 10,
    'legend.loc': 'upper right',
    'legend.labelspacing': 0.25,
    'xtick.labelsize': 12,
    'ytick.labelsize': 12,
    # 'text.usetex': True,
    # 'figure.autolayout': True,
    'ytick.right': False,
    'xtick.top': False,

    'figure.figsize': [12, 10],  # instead of 4.5, 4.5
    'axes.linewidth': 1.5,
    'xtick.major.size': 5,
    'ytick.major.size': 5,
    'xtick.minor.size': 5,
    'ytick.minor.size': 5,

    'xtick.major.width': 3,
    'ytick.major.width': 3,
    'xtick.minor.width': 3,
    'ytick.minor.width': 3,

    'xtick.major.pad': 10,
    'ytick.major.pad': 12,
>>>>>>> c80f4457 (First commit)
    # 'xtick.minor.pad': 14,
    # 'ytick.minor.pad': 14,

    'xtick.direction': 'inout',
    'ytick.direction': 'inout',
<<<<<<< HEAD
=======
    'axes.unicode_minus': False,
>>>>>>> c80f4457 (First commit)
}
plt.clf()
# %%
matplotlib.rcParams.update(params)
map_pro = ccrs.PlateCarree()
<<<<<<< HEAD
Data_path = "/work/users/tamnnm/code/main_code/city_list_obs/city/"
=======
Data_path = "/data/projects/REMOSAT/tamnnm/obs/UPDATE_METEO/"
>>>>>>> c80f4457 (First commit)
os.chdir(Data_path)
city_name = []
city_start_year = []
city_end_year = []
city_lat = []
city_lon = []

# ------------------------------------------------------------------ #
#                    PLOT VIETNAM SUBPLOT STATION                    #
# ------------------------------------------------------------------ #

# region

# ------------------------- Define the file ------------------------ #
<<<<<<< HEAD
# shp_path = "/work/users/tamnnm/geo_info/vnm/full_shp/vnm_admbnda_adm0_gov_20200103.shp"
# vnmap = shp.Reader(shp_path)


# var_long_name = {"rain": "Precipitation",
#                  "vwnd": "V wind",
#                  "uwnd": "U wind",
#                  "temp": "Temp"}
# #number_file=len(os.listdir(Data_path))
# number_file = 4

# for i in range(4,2,-1):
#     if number_file%i == 0:
#         if number_file <= 4:
#             i_new = 2
#         else:
#             i_new = i
#         no_ver = int(i_new)
#         no_hor = int(number_file/i_new)
#     else:
#         continue
# fig,axs = plt.subplots(no_hor,no_ver,subplot_kw=dict(projection=ccrs.PlateCarree()),sharex=True, sharey=True, gridspec_kw={'hspace': 0.1, 'wspace': -0.2})
# #sharex, sharey so that the suplot use the same x-bar or y-bar. The gridspec_kw set to 0 so that it delete the space between the suplots
# for city_path,ax in zip(os.listdir(Data_path),axs.flatten()):
#     """
#     #Create the outer frame
#     ax.set_extent([100,120,5,25])
#     ax.xlabels_top = False
#     ax.ylabels_right = False
#     #ax.coastlines(color='black', linewidth=1, resolution='10m',alpha=0.5)
#     #ax.add_feature(cfeature.BORDERS.with_scale('10m'),linestyle='-')
#     ax.set_xticks(np.linspace(100, 120,5), crs=map_pro)
#     ax.set_yticks(np.linspace(5,25,5), crs=map_pro)
#     lon_formatter = LongitudeFormatter(zero_direction_label=True)
#     lat_formatter = LatitudeFormatter()
#     ax.xaxis.set_major_formatter(lon_formatter)
#     ax.yaxis.set_major_formatter(lat_formatter)
#     ax.tick_params(axis='both',which="minor",labelsize=5)
#     ax.set_xticks(ax.get_xticks())
#     ax.set_yticks(ax.get_yticks())
#     """
#     ax.set_extent([101,120,5,27])
#     #number of station acc
#     station_1=0
#     station_2=0
#     station_3=0
#     var_name=city_path.split('.')[0]
=======

var_long_name = {"rain": "Precipitation",
                 "vwnd": "V wind",
                 "uwnd": "U wind",
                 "temp": "Temp"}
#number_file=len(os.listdir(Data_path))
number_file = 2

no_hor = 1
no_ver = 2

fig,axs = plt.subplots(no_hor,no_ver, figsize=(15,10),subplot_kw=dict(projection=ccrs.PlateCarree()), sharey=True, gridspec_kw={'hspace': 0.1, 'wspace': -0.1})
#sharex, sharey so that the suplot use the same x-bar or y-bar. The gridspec_kw set to 0 so that it delete the space between the suplots
for city_path,ax in zip(['T2m', 'R'],axs.flatten()):
    
    #number of station acc
    station_1=0
    station_2=0
    station_3=0
    var_name=city_path.split('.')[0]
>>>>>>> c80f4457 (First commit)

# METHOD 1:
    # ----------- plot vietnam's boder and the station point ----------- #
#     txt_shapes = []
#     for vnmapshape in vnmap.shapeRecords():
#         listx=[]
#         listy=[]
#         # parts contains end index of each shape part
#         parts_endidx = vnmapshape.shape.parts.tolist()
#         parts_endidx.append(len(vnmapshape.shape.points) - 1)
#         for i in range(len(vnmapshape.shape.points)):
#             x, y = vnmapshape.shape.points[i]
#             if i in parts_endidx:
#                 # we reached end of part/start new part
#                 txt_shapes.append([listx,listy])
#                 listx = [x]
#                 listy = [y]
#             else:
#                 # not end of part
#                 listx.append(x)
#                 listy.append(y)

# ------------- Save the patch geometry to a Shapefile ------------- #
    # patch_polygon = [Polygon(zone) for zone in txt_shapes]
    # patch_gdf = gpd.GeoDataFrame({'geometry': patch_polygon})
    # patch_gdf.to_file("/work/users/tamnnm/geo_info/vnm/vietnam_only.shp")

# #"""
#     for zone in txt_shapes:
#         x,y = zone
#         #Plot only the border
#         ax.plot(x,y,color="k",markersize=10e-6,linewidth=0.4)
#         #Fill the inside with a certainc color
#         #ax.fill(x,y,facecolor='red',edgecolor="k",linewidth=0.4)
#     ax.text(112, 10, "Spratly Islands", fontsize=5,rotation=45)
#     ax.text(111, 14.5, "Paracel Islands", fontsize=5, rotation=45)
#     ax.text(110, 13, "EAST SEA", fontsize=5,rotation=90,alpha=0.6)
#     #ax.text(102,20 , "Laos", color="w", fontsize=16)
#     #ax.text(108,13 , "Vietnam",color="w", fontsize=16,rotation=90)
#     #ax.text(104, 13, "Cambodia",color="w", fontsize=16)
#     ax.grid(linewidth=1, color='gray', alpha=0.5,
#                     linestyle='--')
#     ax.set_title(var_long_name[var_name.split("_")[0]],color='black',fontsize=17)
#     ax.text(112,25.5,"Stating year",fontsize=8,weight='bold')
# #"""

<<<<<<< HEAD
#     # Add open filter file to exclude the station that is not in the list


#     with open(f'{city_path}','r') as file:
#         for i,line in enumerate(file):
#             city_info=line.split(',')
#             print(city_info)
#             city_lon=float(city_info[3])
#             city_lat=float(city_info[4])
#             city_start_year=float(city_info[1])
#             #print(city_lon[0])
#             if city_start_year<=1961:
#                 station_1+=1
#                 ax.plot(city_lon, city_lat,  markersize=2, marker='o', color='red')
#                 #ax.text(115, city_lat,f'{city_info[0]}_{city_info[1]}_{city_info[2]}',color='red',fontsize=5)
#                 #ax.plot([city_lon,city_lon+5],[city_lat, city_lat], linewidth=1, color='r')
#             elif 1961<=city_start_year<=1986:
#                 ax.plot(city_lon, city_lat,  markersize=2, marker='o', color='orange')
#                 station_2+=1
#             else:
#                 ax.plot(city_lon, city_lat,  markersize=2, marker='o', color='black')
#                 station_3+=1
#                 #continue
#     weight = station_1,station_2,station_3
#     """
#     weight_count = {
#         "1961": int(station_1),
#         "1986": int(station_2),
#         "2019": int(station_3)
#     }
#     """
#     name = ['-61','61-86','86-19']
#     color = ['r','orange','black']
#     ins = ax.inset_axes([0.5,0.65,0.4,0.25])
#     p = ins.bar([1,2,3], weight, color = color, tick_label = name, width=0.5, bottom=1)

# fig.suptitle("Station distribution in Vietnam",fontsize=20, y=0.97,x=0.5)
# #fig.tight_layout()
# fig.savefig('/work/users/tamnnm/code/sup/image/map_ob.jpg',format="jpg",bbox_inches='tight',dpi=1000)
=======
    # Add open filter file to exclude the station that is not in the list

    plot_shp(ax, island=True, dem = False, KV = True, KV_fill_color = False)

    # %: Use the original text file
    # with open(Data_path+city_path,'r') as file:
    #     for i,line in enumerate(file):
    #         city_info=line.split(',')
    #         print(city_info)
    #         city_lon=float(city_info[3])
    #         city_lat=float(city_info[4])
    #         city_start_year=float(city_info[1])
    #         #print(city_lon[0])
    #         if city_start_year<=1961:
    #             station_1+=1
    #             ax.plot(city_lon, city_lat,  markersize=2, marker='o', color='red')
    #             #ax.text(115, city_lat,f'{city_info[0]}_{city_info[1]}_{city_info[2]}',color='red',fontsize=5)
    #             #ax.plot([city_lon,city_lon+5],[city_lat, city_lat], linewidth=1, color='r')
    #         elif 1961<=city_start_year<=1986:
    #             ax.plot(city_lon, city_lat,  markersize=2, marker='o', color='orange')
    #             station_2+=1
    #         else:
    #             ax.plot(city_lon, city_lat,  markersize=2, marker='o', color='black')
    #             station_3+=1
    #             #continue
    
    #% Use the nc file
    ds = xr.open_dataset(Data_path+"OG_"+var_name+"_daily_1960_2019.nc")
    for i, no_city in enumerate(ds.no_station.values):
        city_lon = ds.sel(no_station=no_city).lon.values
        city_lat = ds.sel(no_station=no_city).lat.values
        city_start_year = ds.sel(no_station=no_city).start_year.values
        city_end_year = ds.sel(no_station=no_city).end_year.values
        city_gap = ds.sel(no_station=no_city).data_gap.values
        if city_gap <= 0.05 and int(city_end_year) - int(city_start_year) >= 20 and city_start_year <= 1961:
            marker = 'o'
            markersize = 4
        else:
            marker = '+'
            markersize = 4

        if city_start_year <= 1961:
            station_1 += 1
            markercolor = 'red'
        elif 1961 <= city_start_year <= 1986:
            station_2 += 1
            markercolor = 'orange'
        else:
            station_3 += 1
            markercolor = 'black'
    
        ax.plot(city_lon, city_lat, markersize=markersize, marker=marker, color=markercolor)
    weight = station_1,station_2,station_3
    """
    weight_count = {
        "1961": int(station_1),
        "1986": int(station_2),
        "2019": int(station_3)
    }
    """
    name = ['-61','61-86','86-19']
    color = ['r','orange','black']
    ins = ax.inset_axes([0.5,0.65,0.3,0.25])
    p = ins.bar([1,2,3], weight, color = color, tick_label = name, width=0.5, bottom=1)
    
    ax.text(0.05, 0.95, city_path, fontsize=20, transform=ax.transAxes)
#fig.tight_layout()
legend_elements = [mlines.Line2D([0], [0], marker='o', color='w', markeredgecolor='k', label=f'Chosen stations', markerfacecolor='k', markersize=10),
                   mlines.Line2D([0], [0], marker='+', color='w', markeredgecolor='k', label=f'Filtered stations', markerfacecolor='k', markersize=7)]
fig.legend(handles=legend_elements,
        loc='center',
        ncol= 2,
        #? len(markers) if horizontal, 1 if vertical
        bbox_to_anchor=(0.47, 0.25),
        handlelength=1,  # Reduce the length of the legend handles
        handletextpad=0.5,  # Reduce the padding between the handles and the text
        frameon=False,
        fontsize = 15)

fig.savefig('/work/users/tamnnm/code/02.sup/image/map_ob.jpg',format="jpg",bbox_inches='tight',dpi=1000)
>>>>>>> c80f4457 (First commit)

# endregion

# ------------------------------------------------------------------ #
#                     PLOT INTEREST AREA FOR WRF                     #
# ------------------------------------------------------------------ #

# region

# endregion
