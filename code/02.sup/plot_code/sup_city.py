...
fol_grid=['CRU','UDel','GHCN'] #remember that we have to put GHCN at the end to prevent error in plot_full (color run out of range)
#fol_grid=['GHCN']
...
#print(len(daily_max))
def plot_para(z,para,year_to_plot,par):
    fig, ax = plt.subplots()
    year_plot=[]
    plot=[]
    ...
    for i in range(len(para[z].columns)):
        ...
        if year_plot[z][i] in (1943,1944,1945):
         ...
         #the first 3 years (43,44,45) #too-tired to change since HP only have 1947 go on
            line=ax.plot(month,test,alpha=a,color=c,marker='o',label=f'{year_plot[z][i]}')
            plot.append(line)
        #elif i<8:
        else:
            a=0.25
            line=ax.plot(month,test,alpha=a,marker='o',label=f'{year_plot[z][i]}')
            plot.append(line)

    handles, labels = ax.get_legend_handles_labels()
    legend_fig, legend_ax = plt.subplots(figsize=(5,5))
    ...
    legend_fig.savefig(
            f'{Data_path}Data/legend/REAL_{all_para[b]}',
            bbox_inches='tight',
...
for b in range(len(all_para)):
    for a in range(len(pre)):
        ...
        plot_para(a,globals()[all_para[b]],year_plot,f'{all_para[b]}')
...
class var():
    ...
    def var_f(self):
        ...
        for i in range(len(list_var)):
            ...
            if (list_var[i].find("pcp")!=-1) or (list_var[i].find("pre")!=-1) or (list_var[i].find("tp")!=-1):
                var="pre"
    ...
    #Define the factor of the precipitation and maybe other
        if dtset.find("_p")!=-1 or dtset.find("_tp")!=-1:
            if dt=="twcr":
                factor_p=240 #30 days and 8 interval (3-hr interval)
            else:
                factor_p=30000 #30days and 24 hours
        elif dtset=="UDel":
            factor_p=10
        else:
            factor_p=1
...
def data_org(folder):
    if folder==fol_grid:
        ...
        for name in os.listdir(f'{Data_path}cal/'):
            ...
            #if name.find("_tp")==-1 and
            if name.find("_p")==-1 and name.find("_tp")==-1:
                continue
            else:
                #i=i+1
                #print(name)
...
#"""
for dset in (fol_rean+fol_grid):
    if dset in fol_rean:
        #dset_name=f'{dset}_tp'
        if dset!="twcr":
            dset_name=f'{dset}_p'
            dset_name2=f'{dset}_tp'
        else:
            dset_name=f'{dset}_tp'
        #dset_name2=f'{dset}_tp'
    else:
...
def plot_ss(ct,pra,fd,opt):
    ...
    if fd=="grid":
        ...
        for i in range(len(fol_grid)):
            if pra in ("t2m","tmp") and fol_grid[i]=="GHCN":
                ...
                data_TK_mean=data_out(fol_grid[i],pra,(1,2),city=ct,base=b)
                data_TK=data_TK_mean.to_pandas()
                data_TK_plot1=data_TK.reset_index(drop=False)
                if i==0:
                     #time from three dataset is different so we have to reset then use the time of the first to concat
                    data_TK_plot=data_TK_plot1
                else:
                    data_TK_plot=data_TK_plot1.iloc[:,-1]
                #plt.savefig(f'{Data_path}/Data/test/test{i}.jpg',format="jpg")
        ...
        for i in range(len(fol_rean)):
            data_TK_mean=data_out(fol_rean[i],pra,(1,2),city=ct,base=b)
            data_TK=data_TK_mean.to_pandas()
            data_TK_plot1=data_TK.reset_index(drop=False)
            if i==0:
                     #time from three dataset is different so we have to reset then use the time of the first to concat
                ...
                data_TK_plot=data_TK_plot1.iloc[:,-1]
            dt_plt.append(data_TK_plot)   #get the label of all fol_real
    #print(dts)
            #"""

    #custom the x axis for the full plot
    def appear(fig,ax):
    ...
    #plot the real data
        ...
        real_plot=real.reset_index(drop=False)
        dt_plt.append(real_plot.iloc[:,-1])
        sign="Yes"
    except:
        sign="No"
    dt_plt=pd.concat(dt_plt,axis=1).set_index("time")
    ...
    if opt=="full":
        ...
        if sign=="Yes":
            ...
            ax.plot(dts,dt_plt.iloc[:,-1],alpha=1,color="#0000ff",label="obsevered")
            subfol="real"
        else:
            subfol="no_real"
        #"""
        #plot all the fol_grid
        for i in range(len(folder)):
            if folder[i]=="GHCN" and pra=="t2m":
                continue
            else:
                if folder[i]=="twcr":
                    lab="20cr"
                    ...
                    lab=folder[i]
                ax.plot(dts,dt_plt.iloc[:,i],color=col[i],label=lab,alpha=0.8)
            ...
            #The problem: even though having the same time series but when plot it appears a little bit difference from each other
                #to merge/concat use pd.concat (can mix all series, dataframe)
                    ...
                    if folder[i]=="twcr":
                        ...
                        lab=subfol
                    ax2 = ax.twinx()
                    plot_full=ax.plot(dts,dt_plt.iloc[:,i],alpha=0.8,color=col[i],label=lab)
                ...
                #"""
                    ...
                    if pra=="pre":
                        ...
                        ax2.set_ylabel(fr'$T^o$_observed')
                    #turn the label to 90 degrees
                    ax.set_xlabel('year')
                    #fig.legend(loc ='upper left',bbox_to_anchor=(0.13, 0.85))#(handles=lines,labels=labs, loc=0)
                    appear(fig,ax)
                    ...
                    #plt.show()
                    #plt.clf()
                    legend_fig, legend_ax = plt.figure()
                    legend_squared = legend_ax.legend(
        ...
        cef=[]
        rms=[]
        #clean the data for everything that has the nan value due to the original data
        dt_clean=dt_plt.dropna(axis=0)
        sved_ref=std(dt_clean.iloc[:,-1])
        #dt_norm = norm(dt_clean)
        #dt_norm = preprocessing.normalize(dt_clean, axis=0)
        for i in range(len(dt_plt.columns)-1):
            s=std(dt_clean.iloc[:,i])/sved_ref
            c=ccoef(dt_clean.iloc[:,i],dt_clean.iloc[:,-1])
            r=CRMSD(dt_clean.iloc[:,i],dt_clean.iloc[:,-1])/sved_ref
            sved.append(s)
            cef.append(c)
            rms.append(r)
        #print(rms)
        return sved,cef,rms
#REMEMBER
city_real=["DH","DL","DN","H","HN","HT","LC","MC","NT","PK","PP","QN","SG","SJ","SR","V","VC","XK"]
...
def taylor_plot(pra,dts):
    ...
    plt.savefig(os.path.join(Data_path,"Data","TL",subfold,f'{pra}.jpg'),format="jpg")

taylor_plot("t2m","grid")
taylor_plot("t2m","rean")
taylor_plot("pre","grid")
taylor_plot("pre","rean")
#option="full" - all 3" ; "sep" - each one; "tl_grid", "tl_rean"

#plot_ss("HN","pre","grid","full")
#plot_ss("HN","pre","rean","full")
#plot_ss("HN","t2m","grid","full")
#plot_ss("HN","t2m","rean","full")
#"""
#for ct in city_acr:
    #plot_ss(ct,"pre","grid","full") #done
    #plot_ss(ct,"t2m","grid","full") #done
    #plot_ss(ct,"pre","grid","sep") #done
    #plot_ss(ct,"t2m","grid","sep")
