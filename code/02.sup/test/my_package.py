...
def resample(ds,freq: str, method: Literal["sum","mean"] = "sum", closed="left", offset=None):
    # Define the aggeration function based on method
    agg_func = ds.sum if method == "sum" else ds.mean
    ...
    try:
        ds_res = ds.resample(**resample_params).map(agg_func)
    except:
...
def cut_co(dts, ulat, dlat, ulon, dlon, name = [None, None], full: bool = False, data: bool = True):
    ...
    if data:
        ...
        if [ulat,dlat].count(None) == 2:
            ...
            lat_cond=(lat_co >= dlon) & (lat_co <= ulon)
