...
# Set the number of OpenMP threads to the number of available processors
omp_set_num_threads(omp_get_num_procs())
print("Set number of threads")

# Initialize chunk size for scheduling
chunk_size = 0

# Map OpenMP scheduling constants to their string representations
sched_string_map = {int(OMP_SCHED_AUTO): "OMP_SCHED_AUTO"}

# Function to save a variable from a WRF file to a netCDF file
def save_variable(var, wrf_file=wrf_file, cache=cache, out_folder=out_folder, out_name=out_name):
    ...
    # Retrieve the variable from the WRF file
    v = getvar(wrf_file, var, ALL_TIMES, cache=cache)
    var_name = v.name
    print(var_name)

    # Define encoding options for netCDF compression
    encoding = {
        var_name: {
    ...
    try:
        # Save the variable to a netCDF file with the specified encoding
...
# Iterate over the specified OpenMP scheduling policies
for sched in (OMP_SCHED_AUTO,):
    # (OMP_SCHED_STATIC, OMP_SCHED_DYNAMIC,
    #  OMP_SCHED_GUIDED, OMP_SCHED_AUTO):
    
    # Set the OpenMP schedule with the current policy and chunk size
    omp_set_schedule(sched, chunk_size)
    sched_string = sched_string_map[int(sched)]
    print("Running with scheduler: {}".format(sched_string))

    # Measure the time taken to execute the save_variable function in parallel
    start = time()
    with ThreadPoolExecutor() as executor:
        # Submit save_variable tasks to the thread pool executor
